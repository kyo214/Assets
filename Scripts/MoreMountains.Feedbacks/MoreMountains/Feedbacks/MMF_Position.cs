...
public class MMF_Position : MMF_Feedback
{
	...
	protected virtual void ComputeNewCurvePosition(GameObject movingObject, Vector3 initialPosition, float percent, float intensityMultiplier)
	{
		...
		if (RelativePosition)
		{
			_newPosition.x = (AnimateX ? (initialPosition.x + x) : _currentPosition.x);
			_newPosition.y = (AnimateY ? (initialPosition.y + x2) : _currentPosition.y);
			_newPosition.z = (AnimateZ ? (initialPosition.z + x3) : _currentPosition.z);
		}
		else
		{
			_newPosition.x = (AnimateX ? x : _currentPosition.x);
			_newPosition.y = (AnimateY ? x2 : _currentPosition.y);
			_newPosition.z = (AnimateZ ? x3 : _currentPosition.z);
		}
