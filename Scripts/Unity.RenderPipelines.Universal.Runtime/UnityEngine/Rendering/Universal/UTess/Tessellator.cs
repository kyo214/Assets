...
internal struct Tessellator
{
	...
	{
		int lower = ModuleHandle.GetLower<UHull, float2, TestHullPointL>(hulls, hullCount, p, default);
		int upper = ModuleHandle.GetUpper<UHull, float2, TestHullPointU>(hulls, hullCount, p, default);
		if (lower < 0 || upper < 0)
		{
			return false;
		}
	...
	{
		int lower = ModuleHandle.GetLower<UHull, UEvent, TestHullEventLe>(hulls, hullCount, evt, default);
		if (lower < 0)
		{
			return false;
		}
		UHull value = hulls[lower];
		UHull value2 = default;
		value2.a = evt.a;
		value2.b = evt.b;
		value2.idx = evt.idx;
	...
	private bool MergeHulls(NativeArray<UHull> hulls, ref int hullCount, NativeArray<float2> points, UEvent evt)
	{
		...
		evt.a = evt.b;
		evt.b = a;
		int equal = ModuleHandle.GetEqual<UHull, UEvent, TestHullEventE>(hulls, hullCount, evt, default);
		if (equal < 0)
		{
			return false;
		}
	...
	{
		TessEdgeCompare tessEdgeCompare = default;
		bool flag = true;
		int num = 0;
	...
	private unsafe void PrepareDelaunay(NativeArray<int2> edges, int edgeCount)
	{
		...
		edges2.Dispose();
		ModuleHandle.InsertionSort<int2, TessEdgeCompare>(NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(m_Edges), 0, m_Edges.Length - 1, default);
	...
	private int FindConstraint(int a, int b)
	{
		...
		check.x = ((a < b) ? a : b);
		check.y = ((a > b) ? a : b);
		return ModuleHandle.GetEqual<int2, int2, TestEdgePointE>(m_Edges, m_Edges.Length, check, default);
	}
	...
	private int FindNeighbor(NativeArray<int3> cells, int count, int a, int b, int c)
	{
		...
		check.y = y;
		check.z = z;
		return ModuleHandle.GetEqual<int3, int3, TestCellE>(cells, count, check, default);
	}
	...
	private unsafe NativeArray<int3> Constrain(ref int count)
	{
		...
		ModuleHandle.InsertionSort<int3, TessCellCompare>(NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(cells), 0, m_CellCount - 1, default);
	...
	internal unsafe bool Triangulate(NativeArray<float2> points, int pointCount, NativeArray<int2> edges, int edgeCount)
	{
		...
		ModuleHandle.InsertionSort<UEvent, TessEventCompare>(NativeArrayUnsafeUtility.GetUnsafeBufferPointerWithoutChecks(nativeArray), 0, num - 1, default);
		bool flag = true;
		float x = nativeArray[0].a.x - (1f + math.abs(nativeArray[0].a.x)) * math.pow(2f, -16f);
		UHull value6 = default;
		value6.a.x = x;
		value6.a.y = 1f;
		value6.b.x = x;
		value6.b.y = 0f;
