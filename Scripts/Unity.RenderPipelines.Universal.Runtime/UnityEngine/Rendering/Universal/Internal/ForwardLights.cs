...
public class ForwardLights
{
	...
	private void GetViewParams(Camera camera, float4x4 viewToClip, out float viewPlaneBot, out float viewPlaneTop, out float4 viewToViewportScaleBias)
	{
		...
		float2 float7 = (camera.orthographic ? (-math.float2(viewToClip[3][0], viewToClip[3][1])) : math.float2(viewToClip[2][0], viewToClip[2][1]));
	...
	internal unsafe void PreSetup(ref RenderingData renderingData)
	{
		...
		using (new ProfilingScope(null, m_ProfilingSamplerFPSetup))
		{
			...
			int2 int5 = math.int2(cameraData.pixelWidth, cameraData.pixelHeight);
			...
			{
				m_ActualTileWidth <<= 1;
				m_TileResolution = (int5 + m_ActualTileWidth - 1) / m_ActualTileWidth;
			}
			...
			}.ScheduleParallel(m_LightCount * num, 32, default);
			...
			JobHandle dependency3 = new TilingJob
			{
				...
				rangesPerItem = num6,
				worldToViews = worldToViews,
				tileScale = (float2)int5 / (float)m_ActualTileWidth,
				tileScaleInv = (float)m_ActualTileWidth / (float2)int5,
	...
	private void SetupAdditionalLightConstants(CommandBuffer cmd, ref RenderingData renderingData)
	{
		...
		if (num > 0)
		{
			if (m_UseStructuredBuffer)
			{
				...
				int i = 0;
				int num2 = 0;
				ShaderInput.LightData value = default;
