...
public class AdditionalLightsShadowCasterPass : ScriptableRenderPass
{
	...
	internal struct ShadowResolutionRequest(int _visibleLightIndex, int _perLightShadowSliceIndex, int _requestedResolution, bool _softShadow, bool _pointLightShadow)
	{
		public int visibleLightIndex = _visibleLightIndex;

		public int perLightShadowSliceIndex = _perLightShadowSliceIndex;

		public int requestedResolution = _requestedResolution;

		public bool softShadow = _softShadow;

		public bool pointLightShadow = _pointLightShadow;

		public int offsetX = 0;

		public int offsetY = 0;

		public int allocatedResolution = 0;
	}
	...
	private void RenderAdditionalShadowmapAtlas(ref ScriptableRenderContext context, ref RenderingData renderingData)
	{
		...
		{
			ShadowUtils.SetWorldToCameraAndCameraToWorldMatrices(commandBuffer, renderingData.cameraData.GetViewMatrix());
			bool flag2 = false;
			int count = m_ShadowSliceToAdditionalLightIndex.Count;
	...
	internal TextureHandle Render(RenderGraph graph, ref RenderingData renderingData)
	{
		...
		renderGraphBuilder2.SetRenderFunc((PassData data, RenderGraphContext context) =>
		{
			if (data.emptyShadowmap)
			{
