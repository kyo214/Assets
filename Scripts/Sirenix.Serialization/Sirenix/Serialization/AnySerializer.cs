...
public sealed class AnySerializer : Serializer
{
	...
	public override object ReadValueWeak(IDataReader reader)
	{
		...
		bool flag = true;
		EntryType entryType2 = reader.PeekEntry(out var name2);
		if (IsValueType)
		{
			switch (entryType2)
			{
					...
					if (reader.EnterNode(out var type))
					{
						if (type != serializedType)
						{
							if (type != null)
							{
								...
								{
									context.Config.DebugContext.LogWarning("Can't cast serialized type " + type.Name + " into expected type " + serializedType.Name + ". Attempting to deserialize with possibly invalid data. Value may be lost or corrupted for node '" + name2 + "'.");
									return GetBaseFormatter(context.Config.SerializationPolicy).Deserialize(reader);
								}
								context.Config.DebugContext.LogWarning("Can't cast serialized type " + type.Name + " into expected type " + serializedType.Name + ". Value lost for node '" + name2 + "'.");
								return Activator.CreateInstance(SerializedType);
							}
							if (AllowDeserializeInvalidData || reader.Context.Config.AllowDeserializeInvalidData)
							{
								context.Config.DebugContext.LogWarning("Expected complex struct value " + serializedType.Name + " but the serialized type could not be resolved. Attempting to deserialize with possibly invalid data. Value may be lost or corrupted for node '" + name2 + "'.");
								return GetBaseFormatter(context.Config.SerializationPolicy).Deserialize(reader);
							}
		...
		case EntryType.ExternalReferenceByIndex:
		{
			...
			{
				context.Config.DebugContext.LogWarning("Can't cast external reference type " + externalObject2.GetType().Name + " into expected type " + SerializedType.Name + ". Value lost for node '" + name2 + "'.");
				return null;
			}
		...
		case EntryType.ExternalReferenceByGuid:
		{
			...
			{
				context.Config.DebugContext.LogWarning("Can't cast external reference type " + externalObject.GetType().Name + " into expected type " + SerializedType.Name + ". Value lost for node '" + name2 + "'.");
				return null;
			}
		...
		case EntryType.ExternalReferenceByString:
		{
			...
			{
				context.Config.DebugContext.LogWarning("Can't cast external reference type " + externalObject3.GetType().Name + " into expected type " + SerializedType.Name + ". Value lost for node '" + name2 + "'.");
				return null;
			}
		...
		case EntryType.InternalReference:
		{
			...
			{
				context.Config.DebugContext.LogWarning("Can't cast internal reference type " + internalReference.GetType().Name + " into expected type " + SerializedType.Name + ". Value lost for node '" + name2 + "'.");
				return null;
			}
			...
			try
			{
				...
				if (reader.EnterNode(out var type2))
				{
					...
					else
					{
						...
						{
							context.Config.DebugContext.LogWarning("Can't cast serialized type " + type2.Name + " into expected type " + serializedType2.Name + ". Attempting to deserialize with invalid data. Value may be lost or corrupted for node '" + name2 + "'.");
						...
						if (!flag3)
						{
							context.Config.DebugContext.LogWarning("Can't cast serialized type " + type2.Name + " into expected type " + serializedType2.Name + ". Value lost for node '" + name2 + "'.");
							obj2 = null;
						}
					}
					if (currentNodeId >= 0)
