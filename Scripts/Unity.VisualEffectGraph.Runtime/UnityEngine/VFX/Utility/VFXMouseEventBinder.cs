...
internal class VFXMouseEventBinder : VFXEventBinderBase
{
	...
	private void Awake()
	{
		...
		mouseDown.performed += (InputAction.CallbackContext ctx) =>
		{
		...
		mouseUp.performed += (InputAction.CallbackContext ctx) =>
		{
	...
	{
		Ray ray = Camera.main.ScreenPointToRay(GetMousePosition());
		bool flag = GetComponent<Collider>().Raycast(ray, out var _, float.MaxValue);
		if (mouseOver != flag)
		{
			mouseOver = flag;
			if (flag)
