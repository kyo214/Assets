...
public class DefaultContractResolver : IContractResolver
{
	...
	{
		return GetClassHierarchyForType(type).SelectMany((Type baseType) =>
		{
			List<MemberInfo> list = new List<MemberInfo>();
	...
	private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
	{
		...
		if (attribute.ReadData)
		{
			...
			ExtensionDataSetter extensionDataSetter = (object o, string key, object? value) =>
			{
		...
		if (attribute.WriteData)
		{
			...
			ExtensionDataGetter extensionDataGetter = (object o) =>
			{
				object obj = getExtensionDataDictionary(o);
