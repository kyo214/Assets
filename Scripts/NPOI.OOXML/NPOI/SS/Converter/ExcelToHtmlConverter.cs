...
public class ExcelToHtmlConverter
{
	...
	private void BuildStyle_Border(IWorkbook workbook, StringBuilder style, string type, BorderStyle xlsBorder, short borderColor)
	{
		...
		if (workbook is HSSFWorkbook)
		{
			...
			{
				hSSFColor = customPalette.GetColor(borderColor);
			}
			if (hSSFColor != null)
			{
				stringBuilder.Append(' ');
		...
		else
		{
			...
			else
			{
				...
				if (stylesSource != null)
				{
					...
					if (theme != null)
					{
						xSSFColor = theme.GetThemeColor(borderColor);
					}
				}
				if (xSSFColor != null)
				{
					stringBuilder.Append(' ');
					stringBuilder.Append(ExcelToHtmlUtils.GetColor(xSSFColor));
				}
			}
		}
	...
	{
		short boldweight = font.Boldweight;
		if (boldweight != 400 && boldweight == 700)
		{
		...
		if (workbook is HSSFWorkbook)
		{
			...
			if (customPalette != null)
			{
				hSSFColor = customPalette.GetColor(font.Color);
			}
			if (hSSFColor != null)
			{
				style.AppendFormat("color:{0}; ", ExcelToHtmlUtils.GetColor(hSSFColor));
			}
		}
		else
		{
		...
		if (font.FontHeightInPoints != 0.0)
		{
