...
public class XSSFDataValidationConstraint : IDataValidationConstraint
{
	...
	public XSSFDataValidationConstraint(int validationType, int operator1, string formula1, string formula2)
	{
		...
		this.operator1 = operator1;
		Validate();
		if (3 == validationType && formula1 != null && formula1.StartsWith(QUOTE) && formula1.EndsWith(QUOTE))
		{
			string input = formula1.Substring(1, formula1.Length - 2);
			explicitListOfValues = LIST_SPLIT_REGEX.Split(input);
		}
	}
	...
	public string PrettyPrint()
	{
		...
		if (validationType != 0)
		{
			...
			{
				stringBuilder.Append(LIST_SEPARATOR).Append(value).Append(", ");
			}
			if (validationType == 3 && explicitListOfValues != null)
			{
				stringBuilder.Append(QUOTE).Append(Arrays.AsList(explicitListOfValues)).Append(QUOTE)
					.Append(' ');
			}
			else
			{
				stringBuilder.Append(QUOTE).Append(formula1).Append(QUOTE)
					.Append(' ');
			}
			if (formula2 != null)
			{
				stringBuilder.Append(QUOTE).Append(formula2).Append(QUOTE)
					.Append(' ');
			}
		}
