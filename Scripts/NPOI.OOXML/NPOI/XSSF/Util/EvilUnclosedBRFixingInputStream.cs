...
public class EvilUnclosedBRFixingInputStream : Stream
{
	...
	private static byte[] detect = new byte[4] { 60, 98, 114, 62 };
	...
	private int fixUp(byte[] b, int offset, int read)
	{
		for (int i = 0; i < detect.Length - 1; i++)
		{
			...
			if (flag)
			{
				AddToSpare(b, num, i + 1, atTheEnd: true);
				read--;
				read -= i;
				break;
			}
		...
		{
			bool flag2 = true;
			for (int l = 0; (l < detect.Length) & flag2; l++)
			{
				if (b[k + l] != detect[l])
				{
					flag2 = false;
		...
		if (num3 > 0)
		{
			...
			AddToSpare(b, offset + read - num3, num3, atTheEnd: false);
			read -= num3;
		}
