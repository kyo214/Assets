...
internal static class Fx
{
	...
	public static class Tag
	{
		...
		public enum BlocksUsing
		{
			...
			MonitorWait = 1,
			ManualResetEvent = 2,
			AutoResetEvent = MonitorWait | ManualResetEvent,
			AsyncResult = 4,
			IAsyncResult = MonitorWait | AsyncResult,
			PInvoke = ManualResetEvent | AsyncResult,
			InputQueue = AutoResetEvent | AsyncResult,
			ThreadNeutralSemaphore = 8,
			PrivatePrimitive = MonitorWait | ThreadNeutralSemaphore,
			OtherInternalPrimitive = ManualResetEvent | ThreadNeutralSemaphore,
			OtherFrameworkPrimitive = AutoResetEvent | ThreadNeutralSemaphore,
			OtherInterop = AsyncResult | ThreadNeutralSemaphore,
			Other = IAsyncResult | ThreadNeutralSemaphore,
			NonBlocking = PInvoke | ThreadNeutralSemaphore
		}
	...
	private static EtwDiagnosticTrace InitializeTracing()
	{
		...
		{
			etwDiagnosticTrace.RefreshState = (Action)Delegate.Combine(etwDiagnosticTrace.RefreshState, (Action)(() =>
			{
				UpdateLevel();
			}));
		}
