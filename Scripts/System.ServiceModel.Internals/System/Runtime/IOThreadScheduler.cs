...
internal class IOThreadScheduler
{
	...
	private bool ScheduleCallbackHelper(Action<object> callback, object state)
	{
		...
		bool result = slots[(num >> 16) & SlotMask].TryEnqueueWorkItem(callback, state, out var wrapped);
		if (wrapped)
		{
	...
	private bool ScheduleCallbackLowPriHelper(Action<object> callback, object state)
	{
		...
		bool result = slotsLowPri[(num >> 16) & SlotMaskLowPri].TryEnqueueWorkItem(callback, state, out var wrapped);
		if (wrapped)
		{
