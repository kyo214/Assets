...
public struct NetAddress : IEquatable<NetAddress>
{
	...
	{
		if (string.IsNullOrEmpty(ip) || !IPAddress.TryParse(ip, out var _) || port < 0)
		{
			throw new ArgumentException("IP/Port passed are invalid.");
		}
		ip = ip.Split(new char[1] { '%' })[0];
		Address address2 = default;
		Assert.Always(UDP.SetIP(ref address2, ip) == Status.Ok, $"Unable to parse IP: {ip}:{port}. Verify if it represents a valid IP.");
		address2.Port = port;
		return new NetAddress
		{
