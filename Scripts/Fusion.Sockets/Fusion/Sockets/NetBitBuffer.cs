...
public struct NetBitBuffer : INetBitWriteStream, ILogBuilder
{
	...
	public unsafe void ReplaceDataFromBlockWithTemp(int tempSize)
	{
		...
		else
		{
			ulong* data = _data;
			_data = (ulong*)Native.MallocAndClear(tempSize + 1024);
			Native.MemCpy(_data, data, _lengthBytes);
			Native.Free(data);
		}
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private int Advance(int bits, bool writing)
	{
		int offsetBits = _offsetBits;
		_offsetBits += bits;
