...
public struct NetPeerGroup
{
	...
	internal unsafe static void Dispose(NetPeerGroup* g, INetPeerGroupCallbacks callbacks)
	{
		if (g != null)
		{
			NetConnectionMap.Dispose(g->_connectionsMap, callbacks);
			g->_connectionsMap = null;
			NetBitBufferBlock.Dispose(g->_sendBlock);
			g->_sendBlock = null;
			NetBitBufferStack.Free(g->_recvStack);
			g->_recvStack = default;
		}
	}
	...
	public unsafe static void Connect(NetPeerGroup* g, NetAddress address, byte[] token, byte[] uniqueId = null)
	{
		...
		if (ptr == null)
		{
			Log.Error("No free connection slots");
			return;
		}
	...
	public unsafe static void Disconnect(NetPeerGroup* g, NetConnection* c)
	{
		if (g != null && (c->Status == NetConnectionStatus.Connected || c->Status == NetConnectionStatus.Connecting))
		{
			SendCommand(g, c, NetCommandDisconnect.Create(NetDisconnectReason.Requested));
			DisconnectInternal(g, c, NetDisconnectReason.Requested);
		}
	}

	internal unsafe static void DisconnectInternal(NetPeerGroup* g, NetConnection* c, NetDisconnectReason reason = NetDisconnectReason.ByRemote)
	{
		if (g != null && (c->Status == NetConnectionStatus.Connected || c->Status == NetConnectionStatus.Connecting))
		{
	...
	public unsafe static void SendReliable(NetPeerGroup* g, NetConnection* c, int dataKey, byte* data, int dataLength)
	{
		...
		while (dataLength > 0)
		{
			int num = Math.Min(dataLength, 1128);
			ReliableHeader* ptr = (ReliableHeader*)Native.MallocAndClear(32 + num);
			ptr->Key = dataKey;
			ptr->Length = num;
			ptr->Sequence = c->ReliableBuffer.NextSendSequence();
			Native.MemCpy((byte*)ptr + 32, data, num);
			data += num;
			dataLength -= num;
	...
	private unsafe static void SendCommandConnect(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c)
	{
		...
		else
		{
			...
			Log.Debug($"Connection Attempt: {*c} [{c->StateConnecting.Attempts}/{g->_config.ConnectAttempts}]");
		}
	}
	...
	private unsafe static void UpdateConnected(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c)
	{
		...
		if (c->ReliableSendTimer.IsRunning && c->ReliableSendTimer.Peek() >= 0.05 && c->ReliableSendList.Count > 0 && GetNotifyDataBuffer(g, c, out var b))
		{
			c->ReliableSendTimer.Consume();
			ReliableHeader* ptr = c->ReliableSendList.RemoveHead();
			byte* data = ReliableHeader.GetData(ptr);
			b->WriteUInt64(ptr->Sequence, c->ReliableBuffer.SequenceBits);
			b->WriteInt32(ptr->Key);
			Native.MemCpy(b->GetDataPointer(), data, ptr->Length);
			b->OffsetBits += ptr->Length * 8;
			b->PacketType = NetPacketType.NotifyReliableData;
			if (!SendNotifyDataBuffer(g, c, b, ptr))
			{
				Native.Free(ptr);
	...
	private unsafe static void UpdateDisconnected(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c)
	{
		...
		{
			c->StateDisconnected.SentDisconnectCommand = (SendCommand(g, c, NetCommandDisconnect.Create(c->StateDisconnected.Reason)) ? 1 : 0);
		}
	...
	{
		Assert.Check(!c->Address.Equals(default));
		c->SendTime = g->_clock.ElapsedInSeconds;
		c->StatsSentPacketSizes.Push(g->Time, Maths.BytesRequiredForBits(b->OffsetBits));
		b->Mtu = Math.Max(432, c->Mtu);
		IntPtr sendHead;
		do
		{
			sendHead = g->_sendHead;
	...
	public unsafe static bool SendNotifyDataBuffer(NetPeerGroup* g, NetConnection* c, NetBitBuffer* b, void* userData)
	{
		...
		NetSendEnvelope envelope = default;
	...
	private unsafe static void HandlePacketUnconnected(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetBitBuffer* b)
	{
		if (b->PacketType == NetPacketType.Command)
		{
			...
			if (data->Command == NetCommands.Connect)
			{
				...
				if (cb.OnConnectionRequest(b->Address, tokenDataAsArray))
				{
				...
				else if (!SendCommandUnconnected(g, b->Address, NetCommandRefused.Create(NetConnectFailedReason.ServerRefused)))
				{
					Log.DebugError("Sending Refused Connection Failed");
				}
			}
		}
		if (b->PacketType == NetPacketType.Unconnected)
		{
	...
	private unsafe static void HandlePacket(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetBitBuffer* b)
	{
		...
		c->StatsReceivedPacketSizes.Push(g->Time, b->LengthBytes);
		switch (b->PacketType)
		{
		...
		case NetPacketType.Unconnected:
			break;
		default:
			Log.Error($"Invalid Packet Type {b->PacketType}");
			break;
		}
	}
	...
	private unsafe static void HandlePacketNotifyData(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetBitBuffer* b)
	{
		...
		NetNotifyHeader header = default;
		...
		else
		{
			...
			else
			{
				...
				if (b->PacketType == NetPacketType.NotifyReliableData)
				{
					if (c->ReliableBuffer.Receive(b, out var key))
					{
						byte* data = b->GetDataPointer();
						int length = b->LengthBytes - b->OffsetBytes;
						cb.OnReliableData(c, key, data, length);
						void* root;
						while (c->ReliableBuffer.LateReceive(out root, out key, out data, out length))
						{
							cb.OnReliableData(c, key, data, length);
							c->ReliableBuffer.LateFree(root);
						}
					}
				}
				else
	...
	private unsafe static void HandlePacketAcks(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetNotifyHeader h)
	{
		...
		while (c->NotifySendWindow.Count > 0)
		{
			...
			if (num2 == 0)
			{
				...
				c->StatsRoundTripTime.Push(g->Time, c->Rtt);
			}
			num++;
			...
			if (num2 <= -g->_config.Notify.AckMaskBits || (h.AckMask & (ulong)(1L << -num2)) == 0)
			{
				...
				{
					Assert.Check(envelope.UserData);
					ReliableHeader* userData = (ReliableHeader*)envelope.UserData;
					c->ReliableSendList.AddFirst(userData);
				}
				else
				{
					cb.OnNotifyLost(c, envelope);
				}
			}
			...
			{
				cb.OnNotifyDelivered(c, envelope);
			}
		}
		if (num == 0)
		{
	...
	private unsafe static void HandlePacketCommand(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetBitBuffer* b)
	{
		...
		case NetCommands.Disconnect:
			...
			break;
		default:
			Log.Error($"Invalid Command Type {data->Command}");
			break;
		}
	}
	...
	private unsafe static void HandleCommandDisconnect(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetCommandDisconnect cmd)
	{
		if (c->Status != NetConnectionStatus.Connected)
		{
			Log.Error(string.Format("received {0} with connection status {1}", "NetCommandDisconnect", c->Status));
		}
		else
		{
	...
	private unsafe static void HandleCommandConnect(NetPeerGroup* g, INetPeerGroupCallbacks cb, NetConnection* c, NetCommandConnect cmd)
	{
		...
		case NetConnectionStatus.Connected:
			...
			break;
		default:
			Log.Error(string.Format("received {0} with connection status {1}", "NetCommandConnect", c->Status));
			break;
		}
	}
	...
	private unsafe static NetConnection* AllocateConnection(NetPeerGroup* g, NetAddress address, byte[] token, byte[] uniqueId = null)
	{
		NetConnection* ptr = g->_connectionsMap->Insert(address);
		if (ptr == null)
		{
			return null;
		}
		Assert.Check(ptr->RecvTime == 0.0);
		Assert.Check(ptr->SendTime == 0.0);
		Assert.Check(ptr->Rtt == 0.0);
		Assert.Check(ptr->NotifySendSequencer.Sequence == 0);
		Assert.Check(ptr->NotifySendWindow.Head == 0);
		Assert.Check(ptr->NotifySendWindow.Tail == 0);
		Assert.Check(ptr->NotifySendWindow.Count == 0);
		Assert.Check(ptr->NotifyRecvTime == 0.0);
		Assert.Check(ptr->NotifyRecvMask == 0);
		Assert.Check(ptr->NotifyRecvSequence == 0);
		Assert.Check(ptr->NotifyRecvUnackedCount == 0);
		...
		if (uniqueId != null)
		{
			ptr->UniqueId = (byte*)Native.MallocAndClear(uniqueId.Length);
			ptr->UniqueIdHash = BitConverter.ToInt64(uniqueId, 0);
			fixed (byte* source2 = uniqueId)
			{
				Native.MemCpy(ptr->UniqueId, source2, 8);
			}
		}
		return ptr;
	}
