...
public class ExampleUIToggleAnimators : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		if (hasFloatLabel)
		{
			FloatLabel.text = FloatValue.ToString(CultureInfo.InvariantCulture);
		}
		if (hasIntLabel)
		{
			IntLabel.text = IntValue.ToString();
		}
		if (hasVector2Label)
		{
			Vector2Label.text = Vector2Value.ToString();
		}
		if (hasVector3Label)
		{
			Vector3Label.text = Vector3Value.ToString();
		}
	}
}
