...
internal abstract class XmlBaseReader : XmlDictionaryReader
{
	...
	private class NamespaceManager
	{
		...
		public void Sign(XmlSigningNodeWriter writer)
		{
			for (int i = 0; i < nsCount; i++)
			{
				...
				if (!flag)
				{
					byte[] prefixBuffer = prefix.GetString(out var offset, out var length);
					byte[] nsBuffer = namespaces[i].Uri.GetString(out var offset2, out var length2);
	...
	{
		byte[] prefixBuffer = node.Prefix.GetString(out var offset, out var length);
		byte[] localNameBuffer = node.LocalName.GetString(out var offset2, out var length2);
	...
	private void SignAttribute(XmlSigningNodeWriter writer, XmlAttributeNode attributeNode)
	{
		...
		{
			byte[] prefixBuffer = attributeNode.Prefix.GetString(out var offset, out var length);
			byte[] localNameBuffer = attributeNode.LocalName.GetString(out var offset2, out var length2);
		...
		{
			byte[] prefixBuffer2 = attributeNode.Namespace.Prefix.GetString(out var offset3, out var length3);
			byte[] nsBuffer = attributeNode.Namespace.Uri.GetString(out var offset4, out var length4);
