...
internal class XmlBinaryNodeWriter : XmlStreamNodeWriter
{
	...
	{
		byte[] textNodeBuffer = GetTextNodeBuffer(5, out var num);
		if (length < 256)
		{
	...
	{
		byte[] textNodeBuffer = GetTextNodeBuffer(9, out var num);
		textNodeBuffer[num] = (byte)nodeType;
	...
	private unsafe void UnsafeWriteName(char* chars, int charCount)
	{
		if (charCount < 42)
		{
			byte[] array = GetBuffer(1 + charCount * 3, out var num);
	...
	{
		byte[] array = GetBuffer(5, out var num);
		int num2 = num;
	...
	public override void WriteInt32Text(int value)
	{
		if (value >= -128 && value < 128)
		{
			...
			byte[] textNodeBuffer = GetTextNodeBuffer(2, out var num);
		...
		{
			byte[] textNodeBuffer2 = GetTextNodeBuffer(3, out var num2);
		...
		{
			byte[] textNodeBuffer3 = GetTextNodeBuffer(5, out var num3);
	...
	{
		byte[] array = GetBuffer(8, out var num);
	...
	public override void WriteBase64Text(byte[] trailBytes, int trailByteCount, byte[] base64Buffer, int base64Offset, int base64Count)
	{
		...
		if (num > 0)
		{
			...
			if (trailByteCount > 0)
			{
				byte[] array = GetBuffer(trailByteCount, out var num2);
	...
	private unsafe void UnsafeWriteText(char* chars, int charCount)
	{
		...
		if (charCount <= 85)
		{
			byte[] array = GetBuffer(2 + charCount * 3, out var num);
	...
	{
		byte[] textNodeBuffer = GetTextNodeBuffer(17, out var num);
		byte* ptr = (byte*)(&d);
