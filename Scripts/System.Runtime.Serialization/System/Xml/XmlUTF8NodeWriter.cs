...
internal class XmlUTF8NodeWriter : XmlStreamNodeWriter
{
	...
	private class InternalWriteBase64TextAsyncResult : AsyncResult
	{
		...
		private bool HandleWriteCharacters(IAsyncResult result)
		{
			...
			byte[] chars = writer.EndGetBuffer(result, out var charIndex);
	...
	{
		byte[] array = GetBuffer(4, out var num);
		array[num] = 60;
		array[num + 1] = 33;
		array[num + 2] = 45;
		array[num + 3] = 45;
	...
	{
		byte[] array = GetBuffer(3, out var num);
		array[num] = 45;
		array[num + 1] = 45;
		array[num + 2] = 62;
		Advance(3);
	...
	{
		byte[] array = GetBuffer(6, out var num);
		array[num] = 32;
		array[num + 1] = 120;
		array[num + 2] = 109;
		array[num + 3] = 108;
	...
	private void InternalWriteBase64Text(byte[] buffer, int offset, int count)
	{
		...
		while (count >= 3)
		{
			int num = Math.Min(384, count - count % 3);
			int count2 = num / 3 * 4;
			byte[] array = GetBuffer(count2, out var charIndex);
		...
		if (count > 0)
		{
			byte[] array2 = GetBuffer(4, out var charIndex2);
