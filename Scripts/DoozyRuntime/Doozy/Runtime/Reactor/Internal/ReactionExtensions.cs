...
public static class ReactionExtensions
{
	...
	{
		if (target.hasTargetObject)
		{
			Reaction.ReactionByTargetObject.RemoveReaction(target.targetObject, target);
		}
		target.targetObject = targetObject;
		target.hasTargetObject = targetObject != null;
		if (target.hasTargetObject)
		{
			Reaction.ReactionByTargetObject.AddReaction(targetObject, target);
		}
		return target;
	}
	...
	{
		return target.SetTargetObject(null);
	}

	public static T SetObjectId<T>(this T target, object objectId) where T : Reaction
	{
		if (target.hasObjectId)
		{
			Reaction.ReactionByObjectId.RemoveReaction(target.objectId, target);
		}
		target.objectId = objectId;
		target.hasObjectId = objectId != null;
		if (target.hasObjectId)
		{
			Reaction.ReactionByObjectId.AddReaction(objectId, target);
		}
		return target;
	}
	...
	{
		return target.SetObjectId(null);
	}

	public static T SetStringId<T>(this T target, string stringId) where T : Reaction
	{
		if (target.hasStringId)
		{
			Reaction.ReactionByStringId.RemoveReaction(target.stringId, target);
		}
		target.stringId = stringId;
		target.hasStringId = !string.IsNullOrEmpty(stringId);
		if (target.hasStringId)
		{
			Reaction.ReactionByStringId.AddReaction(stringId, target);
		}
		return target;
	}
	...
	{
		return target.SetStringId(string.Empty);
	}
	...
	public static T SetIntId<T>(this T target, int intId) where T : Reaction
	{
		if (target.hasIntId)
		{
			Reaction.ReactionByIntId.RemoveReaction(target.intId, target);
		}
		target.intId = intId;
		target.hasIntId = true;
		if (target.hasIntId)
		{
			Reaction.ReactionByIntId.AddReaction(intId, target);
		}
		return target;
	}
	...
	{
		return target.ClearTargetObject().ClearObjectId().ClearStringId()
			.ClearIntId();
	}
	...
	public static T AddOnPlayCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnPlayCallback = (ReactionCallback)Delegate.Combine(target.OnPlayCallback, callback);
		return target;
	}
	...
	public static T AddOnStopCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnStopCallback = (ReactionCallback)Delegate.Combine(target.OnStopCallback, callback);
		return target;
	}
	...
	public static T AddOnFinishCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnFinishCallback = (ReactionCallback)Delegate.Combine(target.OnFinishCallback, callback);
		return target;
	}
	...
	public static T AddOnLoopCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnLoopCallback = (ReactionCallback)Delegate.Combine(target.OnLoopCallback, callback);
		return target;
	}
	...
	public static T AddOnPauseCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnPauseCallback = (ReactionCallback)Delegate.Combine(target.OnPauseCallback, callback);
		return target;
	}
	...
	public static T AddOnResumeCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnResumeCallback = (ReactionCallback)Delegate.Combine(target.OnResumeCallback, callback);
		return target;
	}
	...
	public static T AddOnUpdateCallback<T>(this T target, ReactionCallback callback) where T : Reaction
	{
		...
		target.OnUpdateCallback = (ReactionCallback)Delegate.Combine(target.OnUpdateCallback, callback);
		return target;
	}
	...
	{
		return target.ClearOnFinishCallback().ClearOnLoopCallback().ClearOnPauseCallback()
			.ClearOnPlayCallback()
