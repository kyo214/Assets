...
internal sealed class XPathDocumentBuilder : XmlRawWriter
{
	...
	public void Initialize(XPathDocument doc, IXmlLineInfo lineInfo, string baseUri, XPathDocument.LoadFlags flags)
	{
		...
		int idxText = NewNode(out var page, XPathNodeType.Text, string.Empty, string.Empty, string.Empty, string.Empty);
	...
	internal override void WriteNamespaceDeclaration(string prefix, string namespaceName)
	{
		...
		int num2 = NewNamespaceNode(out var page, prefix, namespaceName, _pageParent, _idxParent);
		if (num != 0)
		{
	...
	private void AddSibling(XPathNodeType xptyp, string localName, string namespaceUri, string prefix, string baseUri)
	{
		...
		int idxSibling = NewNode(out var page, xptyp, localName, namespaceUri, prefix, baseUri);
		if (_idxParent != 0)
		{
