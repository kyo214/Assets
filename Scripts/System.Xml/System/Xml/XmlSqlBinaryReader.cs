...
internal sealed class XmlSqlBinaryReader : XmlReader, IXmlNamespaceResolver
{
	...
	private string ParseText()
	{
		...
		try
		{
			...
			int cch = ScanText(out var start);
	...
	private void HashCheckForDuplicateAttributes()
	{
		...
		for (int i = 0; i < attrCount; i++)
		{
			int localnameAndNamespaceUriAndHash = attributes[i].GetLocalnameAndNamespaceUriAndHash(hasher, out var localname, out var namespaceUri);
