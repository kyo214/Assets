...
internal class QilGenerator : IErrorHelper, IXPathEnvironment, IFocus
{
	...
	private QilList BuildDebuggerNamespaces()
	{
		if (IsDebug)
		{
			...
			foreach (CompilerScopeManager<QilIterator>.ScopeRecord item in scope)
			{
	...
	private QilNode CompileElement(NodeCtor node)
	{
		...
		else
		{
			...
			bool flag = compiler.ParseQName(qname, out var prefix, out var localName, this);
	...
	private QilNode CompileAttribute(NodeCtor node)
	{
		...
		else
		{
			...
			bool flag2 = compiler.ParseQName(text, out var prefix, out var localName, this);
			string text2;
			if (qilNode == null)
			{
