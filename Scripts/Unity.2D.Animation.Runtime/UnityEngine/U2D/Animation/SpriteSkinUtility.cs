...
internal static class SpriteSkinUtility
{
	internal static SpriteSkinValidationResult Validate(this SpriteSkin spriteSkin)
	{
		...
		if (!BurstedSpriteSkinUtilities.ValidateBoneWeights(spriteSkin.spriteBoneWeights, length))
		{
	...
	{
		float3 float5 = deformablePositions[0];
		float3 float6 = deformablePositions[0];
		for (int i = 1; i < deformablePositions.Length; i++)
		{
			float5 = math.min(float5, deformablePositions[i]);
			float6 = math.max(float6, deformablePositions[i]);
		}
		float3 float7 = (float6 - float5) * 0.5f;
		float3 float8 = float5 + float7;
		return new Bounds
		{
			center = float8,
			extents = float7
