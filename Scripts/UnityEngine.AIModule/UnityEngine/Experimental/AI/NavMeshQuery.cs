...
public struct NavMeshQuery : IDisposable
{
	...
	{
		PathQueryStatus closestPointOnPoly = GetClosestPointOnPoly(m_NavMeshQuery, polygon, position, out var nearest);
	...
	public unsafe PathQueryStatus Raycast(out NavMeshHit hit, NavMeshLocation start, Vector3 targetPosition, int areaMask = -1, NativeArray<float> costs = default(NativeArray<float>))
	{
		...
		PathQueryStatus pathQueryStatus = Raycast(m_NavMeshQuery, start, targetPosition, areaMask, costs2, out hit, null, out var _, 0);
	...
	[MethodImpl(MethodImplOptions.InternalCall)]
	private static extern void MapLocation_Injected(IntPtr navMeshQuery, ref Vector3 position, ref Vector3 extents, int agentTypeID, [Optional][DefaultParameterValue(-1)] int areaMask, out NavMeshLocation ret);
