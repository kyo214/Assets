...
public class BlockingCollection<T> : IEnumerable<T>, IEnumerable, ICollection, IDisposable, IReadOnlyCollection<T>
{
	...
	{
		TryAddWithNoTimeValidation(item, -1, default);
	}
	...
	{
		return TryAddWithNoTimeValidation(item, 0, default);
	}

	public bool TryAdd(T item, TimeSpan timeout)
	{
		ValidateTimeout(timeout);
		return TryAddWithNoTimeValidation(item, (int)timeout.TotalMilliseconds, default);
	}

	public bool TryAdd(T item, int millisecondsTimeout)
	{
		ValidateMillisecondsTimeout(millisecondsTimeout);
		return TryAddWithNoTimeValidation(item, millisecondsTimeout, default);
	}
	...
	private static int TryAddToAnyCore(BlockingCollection<T>[] collections, T item, int millisecondsTimeout, CancellationToken externalCancellationToken)
	{
		...
		List<WaitHandle> handles = GetHandles(collections, externalCancellationToken, isAddOperation: true, out var cancellationTokens);
	...
	private static int TryTakeFromAnyCoreSlow(BlockingCollection<T>[] collections, out T item, int millisecondsTimeout, bool isTakeOperation, CancellationToken externalCancellationToken)
	{
		...
		while (millisecondsTimeout == -1 || num >= 0)
		{
			List<WaitHandle> handles = GetHandles(collections, externalCancellationToken, isAddOperation: false, out var cancellationTokens);
