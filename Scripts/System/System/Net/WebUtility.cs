		...
		for (int i = 0; i < length; i++)
		{
			...
			if (c == '&')
			{
				...
				if (num > 0 && value[num] == ';')
				{
					...
					{
						bool flag = ((text[1] != 'x' && text[1] != 'X') ? uint.TryParse(text.Substring(1), NumberStyles.Integer, NumberFormatInfo.InvariantInfo, out var result) : uint.TryParse(text.Substring(2), NumberStyles.AllowHexSpecifier, NumberFormatInfo.InvariantInfo, out result));
						if (flag)
						{
