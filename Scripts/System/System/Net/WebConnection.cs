...
internal class WebConnection : IDisposable
{
	...
	private async Task Connect(WebOperation operation, CancellationToken cancellationToken)
	{
		...
		foreach (IPAddress iPAddress in addressList)
		{
			...
			{
				operation.ThrowIfDisposed(cancellationToken);
				await Task.Factory.FromAsync((IPEndPoint targetEndPoint, AsyncCallback callback, object state) => ((Socket)state).BeginConnect(targetEndPoint, callback, state), (IAsyncResult asyncResult) =>
				{
