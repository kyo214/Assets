...
public static class SocketTaskExtensions
{
	...
	{
		return Task.Factory.FromAsync((EndPoint targetEndPoint, AsyncCallback callback, object state) => ((Socket)state).BeginConnect(targetEndPoint, callback, state), (IAsyncResult asyncResult) =>
		{
	...
	{
		return Task.Factory.FromAsync((IPAddress targetAddress, int targetPort, AsyncCallback callback, object state) => ((Socket)state).BeginConnect(targetAddress, targetPort, callback, state), (IAsyncResult asyncResult) =>
		{
			((Socket)asyncResult.AsyncState).EndConnect(asyncResult);
		}, address, port, socket);
	}
	...
	{
		return Task.Factory.FromAsync((IPAddress[] targetAddresses, int targetPort, AsyncCallback callback, object state) => ((Socket)state).BeginConnect(targetAddresses, targetPort, callback, state), (IAsyncResult asyncResult) =>
		{
			((Socket)asyncResult.AsyncState).EndConnect(asyncResult);
	...
	{
		return Task.Factory.FromAsync((string targetHost, int targetPort, AsyncCallback callback, object state) => ((Socket)state).BeginConnect(targetHost, targetPort, callback, state), (IAsyncResult asyncResult) =>
		{
			((Socket)asyncResult.AsyncState).EndConnect(asyncResult);
		}, host, port, socket);
	}
	...
	public static Task<SocketReceiveFromResult> ReceiveFromAsync(this Socket socket, ArraySegment<byte> buffer, SocketFlags socketFlags, EndPoint remoteEndPoint)
	{
		...
		return Task<SocketReceiveFromResult>.Factory.FromAsync((ArraySegment<byte> targetBuffer, SocketFlags flags, AsyncCallback callback, object obj) =>
		{
	...
	public static Task<SocketReceiveMessageFromResult> ReceiveMessageFromAsync(this Socket socket, ArraySegment<byte> buffer, SocketFlags socketFlags, EndPoint remoteEndPoint)
	{
		...
		return Task<SocketReceiveMessageFromResult>.Factory.FromAsync((ArraySegment<byte> targetBuffer, AsyncCallback callback, object obj) =>
		{
			object[] array = (object[])obj;
			Socket socket2 = (Socket)array[0];
	...
	public static ValueTask<int> ReceiveAsync(this Socket socket, Memory<byte> memory, SocketFlags socketFlags, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		socket.BeginReceive(buffer, 0, memory.Length, socketFlags, (IAsyncResult iar) =>
		{
