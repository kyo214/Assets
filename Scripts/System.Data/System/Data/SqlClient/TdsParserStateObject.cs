...
internal abstract class TdsParserStateObject
{
	...
	internal Task ExecuteFlush()
	{
		lock (this)
		{
			...
			return AsyncHelper.CreateContinuationTask(task, () =>
			{
				_pendingData = true;
				_messageStatus = 0;
			});
	...
	internal int ReadPlpBytesChunk(byte[] buff, int offset, int len)
	{
		...
		bool num2 = TryReadByteArray(buff, offset, num, out var totalRead);
	...
	private void OnTimeout(object state)
	{
		...
		lock (this)
		{
			...
			Task.Delay(5000).ContinueWith((Task _) =>
			{
		...
		finally
		{
			...
			if (flag && source != null && num < 2)
			{
				if (error == 0)
				{
					...
					{
						ExecutionContext.Run(_executionContext, (object state) =>
						{
	...
	private Task WriteSni(bool canAccumulate)
	{
		...
		Task result = SNIWritePacket(resetWritePacket, out var _, canAccumulate, callerHasConnectionLock: true);
