...
internal sealed class SqlSequentialTextReader : TextReader
{
		...
		else
		{
			try
			{
				...
				else
				{
					...
					byte[] byteBuffer = PrepareByteBuffer(charsNeeded, out var byteBufferUsed);
					if (byteBufferUsed < byteBuffer.Length || byteBuffer.Length == 0)
					{
						SqlDataReader reader = _reader;
						if (reader != null)
						{
							Task<int> bytesAsync = reader.GetBytesAsync(_columnIndex, byteBuffer, byteBufferUsed, byteBuffer.Length - byteBufferUsed, -1, _disposalTokenSource.Token, out var bytesRead);
							if (bytesAsync == null)
							{
							...
							{
								flag = false;
								bytesAsync.ContinueWith((Task<int> t) =>
								{
									_currentTask = null;
	...
	private int InternalRead(char[] buffer, int index, int count)
	{
		try
		{
			byte[] array = PrepareByteBuffer(count, out var byteBufferUsed);
