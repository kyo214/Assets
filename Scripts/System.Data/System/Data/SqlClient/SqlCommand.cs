...
public sealed class SqlCommand : DbCommand, ICloneable, IDbCommand, IDisposable
{
	...
	public IAsyncResult BeginExecuteNonQuery(AsyncCallback callback, object stateObject)
	{
		...
		try
		{
			...
			try
			{
				...
				if (task != null)
				{
					AsyncHelper.ContinueTask(task, completion, () =>
					{
			...
			if (callback != null)
			{
				completion.Task.ContinueWith((Task<object> t) =>
				{
					callback(t);
	...
	public override Task<int> ExecuteNonQueryAsync(CancellationToken cancellationToken)
	{
		...
		if (cancellationToken.CanBeCanceled)
		{
			...
			registration = cancellationToken.Register((object s) =>
			{
		...
		try
		{
			...
			Task<int>.Factory.FromAsync(BeginExecuteNonQuery, EndExecuteNonQuery, null).ContinueWith((Task<int> t) =>
			{
	...
	{
		return ExecuteReaderAsync(behavior, cancellationToken).ContinueWith((Func<Task<SqlDataReader>, DbDataReader>)((Task<SqlDataReader> result) =>
		{
			if (result.IsFaulted)
			{
	...
	public new Task<SqlDataReader> ExecuteReaderAsync(CommandBehavior behavior, CancellationToken cancellationToken)
	{
		...
		{
			RegisterForConnectionCloseNotification(ref outerTask);
			Task<SqlDataReader>.Factory.FromAsync(BeginExecuteReader, EndExecuteReader, behavior, null).ContinueWith((Task<SqlDataReader> t) =>
			{
				registration.Dispose();
				if (t.IsFaulted)
				{
	...
	public override Task<object> ExecuteScalarAsync(CancellationToken cancellationToken)
	{
		...
		return ExecuteReaderAsync(cancellationToken).ContinueWith((Task<SqlDataReader> executeTask) =>
		{
			...
			else
			{
				...
				reader.ReadAsync(cancellationToken).ContinueWith((Task<bool> readTask) =>
				{
					try
					{
	...
	public Task<XmlReader> ExecuteXmlReaderAsync(CancellationToken cancellationToken)
	{
		...
		{
			RegisterForConnectionCloseNotification(ref outerTask);
			Task<XmlReader>.Factory.FromAsync(BeginExecuteXmlReader, EndExecuteXmlReader, null).ContinueWith((Task<XmlReader> t) =>
			{
				registration.Dispose();
				if (t.IsFaulted)
				{
				...
				if (async)
				{
					...
					AsyncHelper.ContinueTask(task, completion, () =>
					{
						if (!completion.Task.IsCompleted)
						{
							...
							{
								AsyncHelper.ContinueTask(task2, completion, () =>
								{
									completion.SetResult(null);
								});
							}
	...
	private SqlDataReader RunExecuteReaderTds(CommandBehavior cmdBehavior, RunBehavior runBehavior, bool returnStream, bool async, int timeout, out Task task, bool asyncWrite, SqlDataReader ds = null)
	{
		...
		if (task2 != null)
		{
			...
			if (async)
			{
				...
				AsyncHelper.ContinueTask(task2, completion, () =>
				{
					if (!completion.Task.IsCompleted)
					{
						...
						{
							AsyncHelper.ContinueTask(task4, completion, () =>
							{
								completion.SetResult(null);
							});
						}
			...
			AsyncHelper.WaitForCompletion(task2, timeout, () =>
			{
				throw SQL.CR_ReconnectTimeout();
			});
			timeout = TdsParserStaticMethods.GetRemainingTimeout(timeout, reconnectionStart);
		...
		try
		{
			...
			if (async)
			{
				...
				if (task3 != null)
				{
					task = AsyncHelper.CreateContinuationTask(task3, () =>
					{
