...
public sealed class SqlBulkCopy : IDisposable
{
	...
	private Task ReadFromRowSourceAsync(CancellationToken cts)
	{
		...
		{
			return _DbDataReaderRowSource.ReadAsync(cts).ContinueWith((Task<bool> t) =>
			{
	...
	private Task WriteRowSourceToServerAsync(int columnCount, CancellationToken ctoken)
	{
		...
		if (currentReconnectionTask != null && !currentReconnectionTask.IsCompleted)
		{
			if (_isAsyncBulkCopy)
			{
				...
				currentReconnectionTask.ContinueWith((Task t) =>
				{
					...
					{
						AsyncHelper.ContinueTask(task2, tcs, () =>
						{
							tcs.SetResult(null);
						});
					}
			...
			AsyncHelper.WaitForCompletion(currentReconnectionTask, BulkCopyTimeout, () =>
			{
	...
	{
		object obj = GetValueFromSourceRow(col, out var isSqlType, out var isDataFeed, out var isNull);
			...
			for (i = rowsSoFar; totalRows <= 0 || i < totalRows; i++)
			{
				...
				if (task2 == null)
				{
					...
					if (task3 != null)
					{
						...
						task = source.Task;
						AsyncHelper.ContinueTask(task3, source, () =>
						{
				...
				task = source.Task;
				AsyncHelper.ContinueTask(task2, source, () =>
				{
					CheckAndRaiseNotification();
					...
					{
						AsyncHelper.ContinueTask(task4, source, () =>
						{
							CopyRowsAsync(i + 1, totalRows, cts, source);
						}, _connection.GetOpenTdsConnection());
					}
	...
	private Task CopyBatchesAsyncContinued(BulkCopySimpleResultSet internalResults, string updateBulkCommandText, CancellationToken cts, TaskCompletionSource<object> source)
	{
		try
		{
			...
			if (task != null)
			{
				...
				}, _connection.GetOpenTdsConnection(), (Exception _) =>
				{
	...
	private void WriteToServerInternalRestAsync(CancellationToken cts, TaskCompletionSource<object> source)
	{
		...
		try
		{
			...
			try
			{
				task = _connection.ValidateAndReconnect(() =>
				{
					if (_parserLock != null)
					{
						_parserLock.Release();
			...
			if (task != null)
			{
				if (_isAsyncBulkCopy)
				{
					...
					{
						regReconnectCancel = cts.Register((object s) =>
						{
							((TaskCompletionSource<object>)s).TrySetCanceled();
						}, cancellableReconnectTS);
					}
					AsyncHelper.ContinueTask(task, cancellableReconnectTS, () =>
					{
				...
				try
				{
					AsyncHelper.WaitForCompletion(task, BulkCopyTimeout, () =>
					{
						throw SQL.CR_ReconnectTimeout();
					});
				}
