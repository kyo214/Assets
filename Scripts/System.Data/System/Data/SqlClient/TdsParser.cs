...
internal sealed class TdsParser
{
	...
	{
		SNIErrorDetails result = default;
		if (TdsParserStateObjectFactory.UseManagedSNI)
		{
	...
	internal void ThrowExceptionAndWarning(TdsParserStateObject stateObj, bool callerHasConnectionLock = false, bool asyncClose = false)
	{
		...
		SqlErrorCollection fullErrorAndWarningCollection = stateObj.GetFullErrorAndWarningCollection(out var broken);
		...
		if (asyncClose)
		{
			...
			Action<Action> wrapCloseInAction = (Action closeAction) =>
			{
	...
	{
		multiPartTableName = default;
		MultiPartTableName multiPartTableName2 = default;
		if (!stateObj.TryReadByte(out var value))
		{
			return false;
		}
			...
			try
			{
				...
				for (int ii = startRpc; ii < rpcArray.Length; ii++)
				{
					...
					for (i = ((ii == startRpc) ? startParam : 0); i < parameters.Length; i++)
					{
						...
						}, _connHandler, (Exception exc) =>
						{
							TdsExecuteRPC_OnFailure(exc, stateObj);
						});
						if (flag2)
						{
							task2.ContinueWith((Task _) =>
							{
								_connHandler._parserLock.Release();
							}, TaskScheduler.Default);
							flag2 = false;
	...
	{
		SmiParameterMetaData smiParameterMetaData = param.MetaDataForSmi(out var peekAhead);
		if (!_isKatmai)
		{
