...
public static class UTF8ArrayUnsafeUtility
{
	...
	{
		ConversionError num = Unicode.Utf16ToUtf8(src, srcLength, dest, out var utf8Length, destUTF8MaxLengthInBytes);
	...
	{
		ConversionError num = Unicode.Utf8ToUtf8(src, srcLength, dest, out var destLength2, destUTF8MaxLengthInBytes);
	...
	{
		ConversionError num = Unicode.Utf8ToUtf16(src, srcLength, dest, out var utf16Length, destUCS2MaxLengthInChars);
	...
	{
		ConversionError num = Unicode.Utf8ToUtf8(src, srcLength, dest + (int)destLength, out var destLength2, destUTF8MaxLengthInBytes - destLength);
		destLength += (ushort)destLength2;
		if (num == ConversionError.None)
		{
			return CopyError.None;
	...
	{
		ConversionError num = Unicode.Utf16ToUtf8(src, srcLength, dest + (int)destLength, out var utf8Length, destUTF8MaxLengthInBytes - destLength);
		destLength += (ushort)utf8Length;
		if (num == ConversionError.None)
		{
			return CopyError.None;
	...
	{
		ConversionError num = Unicode.Utf8ToUtf16(src, srcLength, dest + (int)destLength, out var utf16Length, destUCS2MaxLengthInChars - destLength);
		destLength += (ushort)utf16Length;
		if (num == ConversionError.None)
		{
			return CopyError.None;
	...
	public unsafe static int StrCmp(byte* utf8BufferA, int utf8LengthInBytesA, byte* utf8BufferB, int utf8LengthInBytesB)
	{
		...
		do
		{
			ConversionError errorA = Unicode.Utf8ToUcs(out var rune, utf8BufferA, ref index, utf8LengthInBytesA);
			ConversionError errorB = Unicode.Utf8ToUcs(out var rune2, utf8BufferB, ref index2, utf8LengthInBytesB);
	...
	public unsafe static int StrCmp(char* utf16BufferA, int utf16LengthInCharsA, char* utf16BufferB, int utf16LengthInCharsB)
	{
		...
		do
		{
			ConversionError errorA = Unicode.Utf16ToUcs(out var rune, utf16BufferA, ref index, utf16LengthInCharsA);
			ConversionError errorB = Unicode.Utf16ToUcs(out var rune2, utf16BufferB, ref index2, utf16LengthInCharsB);
			comparison = new Comparison(rune, errorA, rune2, errorB);
		}
		while (!comparison.terminates);
		return comparison.result;
	...
	public unsafe static int StrCmp(byte* utf8Buffer, int utf8LengthInBytes, char* utf16Buffer, int utf16LengthInChars)
	{
		...
		do
		{
			ConversionError errorA = Unicode.Utf8ToUcs(out var rune, utf8Buffer, ref index, utf8LengthInBytes);
			ConversionError errorB = Unicode.Utf16ToUcs(out var rune2, utf16Buffer, ref index2, utf16LengthInChars);
			comparison = new Comparison(rune, errorA, rune2, errorB);
		}
		while (!comparison.terminates);
		return comparison.result;
