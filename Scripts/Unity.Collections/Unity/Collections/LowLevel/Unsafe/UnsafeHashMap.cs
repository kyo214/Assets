...
public struct UnsafeHashMap<TKey, TValue> : INativeDisposable, IDisposable, IEnumerable<KeyValue<TKey, TValue>>, IEnumerable where TKey : struct, IEquatable<TKey> where TValue : struct
{
	...
	public unsafe TValue this[TKey key]
	{
		...
		set
		{
			if (UnsafeHashMapBase<TKey, TValue>.TryGetFirstValueAtomic(m_Buffer, key, out var _, out var it))
			{
				UnsafeHashMapBase<TKey, TValue>.SetValue(m_Buffer, ref it, ref value);
			}
			else
			{
				UnsafeHashMapBase<TKey, TValue>.TryAdd(m_Buffer, key, value, isMultiHashMap: false, m_AllocatorLabel);
			}
		}
	}
	...
	public unsafe bool TryAdd(TKey key, TValue item)
	{
		return UnsafeHashMapBase<TKey, TValue>.TryAdd(m_Buffer, key, item, isMultiHashMap: false, m_AllocatorLabel);
	}
	...
	{
		return UnsafeHashMapBase<TKey, TValue>.Remove(m_Buffer, key, isMultiHashMap: false) != 0;
	}
	...
	{
		NativeArray<TKey> result = CollectionHelper.CreateNativeArray<TKey>(UnsafeHashMapData.GetCount(m_Buffer), allocator, NativeArrayOptions.UninitializedMemory);
	...
	{
		NativeArray<TValue> result = CollectionHelper.CreateNativeArray<TValue>(UnsafeHashMapData.GetCount(m_Buffer), allocator, NativeArrayOptions.UninitializedMemory);
	...
	{
		NativeKeyValueArrays<TKey, TValue> result = new NativeKeyValueArrays<TKey, TValue>(UnsafeHashMapData.GetCount(m_Buffer), allocator, NativeArrayOptions.UninitializedMemory);
