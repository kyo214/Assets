...
public static class Extensions
{
	...
	{
		T[] items = System.Collections.Generic.EnumerableHelpers.ToArray(source, out var count);
		if (count > 0)
		{
	...
	public static void Remove(this IEnumerable<XAttribute> source)
	{
		...
		XAttribute[] array = System.Collections.Generic.EnumerableHelpers.ToArray(source, out var length);
	...
	public static void Remove<T>(this IEnumerable<T> source) where T : XNode
	{
		...
		T[] array = System.Collections.Generic.EnumerableHelpers.ToArray(source, out var length);
		for (int i = 0; i < length; i++)
		{
			array[i]?.Remove();
		}
