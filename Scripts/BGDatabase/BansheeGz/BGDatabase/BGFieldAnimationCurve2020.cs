...
public class BGFieldAnimationCurve2020 : BGFieldUnityClassA<AnimationCurve>
{
	...
	private struct KeyframeModel(Keyframe key)
	{
		...
		public int weightedMode = (int)key.weightedMode;

		public float inWeight = key.inWeight;

		public float outWeight = key.outWeight;

		public Keyframe ToKeyframe()
		{
			Keyframe result = new Keyframe(time, value, inTangent, outTangent, inWeight, outWeight);
			result.tangentMode = tangentMode;
			result.weightedMode = (WeightedMode)weightedMode;
			return result;
		}
	}
	...
	public BGFieldAnimationCurve2020(BGMetaEntity meta, string name)
		: base(meta, name)
	{
	}

	internal BGFieldAnimationCurve2020(BGMetaEntity meta, BGId id, string name)
		: base(meta, id, name)
	{
	}
	...
	{
		return (BGMetaEntity meta, BGId id, string name) => new BGFieldAnimationCurve2020(meta, id, name);
	}
	...
	protected override AnimationCurve FromBytes(ArraySegment<byte> segment)
	{
		...
		reader.ReadArray(() =>
		{
			Keyframe item = new Keyframe(reader.ReadFloat(), reader.ReadFloat(), reader.ReadFloat(), reader.ReadFloat(), reader.ReadFloat(), reader.ReadFloat())
			{
	...
	protected override void ToBytes(BGBinaryWriter writer, AnimationCurve value)
	{
		...
		writer.AddArray(() =>
		{
			...
			for (int i = 0; i < keys2.Length; i++)
			{
				...
				writer.AddFloat(keyframe.inWeight);
				writer.AddFloat(keyframe.outWeight);
				writer.AddInt(keyframe.tangentMode);
				writer.AddInt((int)keyframe.weightedMode);
			}
		}, count);
	}
	...
	public static bool Equals(AnimationCurve myValue, AnimationCurve otherValue)
	{
		...
		for (int i = 0; i < keys.Length; i++)
		{
			...
			if (!Mathf.Approximately(keyframe.inWeight, keyframe2.inWeight))
			{
				return false;
			}
			if (!Mathf.Approximately(keyframe.outWeight, keyframe2.outWeight))
			{
				return false;
			}
			...
			if (keyframe.weightedMode != keyframe2.weightedMode)
			{
				return false;
			}
		}
	...
	public static AnimationCurve Clone(AnimationCurve curve)
	{
		...
		else
		{
			...
			{
				Keyframe keyframe = keys[i];
				array[i] = new Keyframe(keyframe.time, keyframe.value, keyframe.inTangent, keyframe.outTangent, keyframe.inWeight, keyframe.outWeight)
				{
