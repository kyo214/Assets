using System;

namespace BansheeGz.BGDatabase;

[FieldDescriptor(Name = "listGuid", Folder = "List/Primitive", ManagerType = "BansheeGz.BGDatabase.Editor.BGFieldManagerListGuid")]
public class BGFieldListGuid : BGFieldCachedStructListA<Guid>
{
	public const ushort CodeType = 16;

	public override ushort TypeCode => 16;

	protected override int ValueSize => 16;

	public BGFieldListGuid(BGMetaEntity meta, string name)
		: base(meta, name)
	{
	}

	internal BGFieldListGuid(BGMetaEntity meta, BGId id, string name)
		: base(meta, id, name)
	{
	}

	protected override byte[] ValueToBytes(Guid value)
	{
		return BGFieldGuid.ValueToBytes(value);
	}

	protected override Guid ValueFromBytes(ArraySegment<byte> segment)
	{
		return BGFieldGuid.ValueFromBytes(segment);
	}

	protected override Guid ValueFromBytes(byte[] array, int offset)
	{
		byte[] array2 = new byte[16];
		Buffer.BlockCopy(array, offset, array2, 0, 16);
		return new Guid(array2);
	}

	protected override string ValueToString(Guid value)
	{
		return BGFieldGuid.ValueToString(value);
	}

	protected override Guid ValueFromString(string value)
	{
		return BGFieldGuid.ValueFromString(value);
	}

	protected override Func<BGMetaEntity, BGId, string, BGField> CreateFieldFactory()
	{
		return (BGMetaEntity meta, BGId id, string name) => new BGFieldListGuid(meta, id, name);
	}

	protected override bool AreEqual(Guid myValue, Guid myValue2)
	{
		return myValue == myValue2;
	}
}
