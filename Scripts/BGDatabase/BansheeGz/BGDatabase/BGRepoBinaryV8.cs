...
public class BGRepoBinaryV8 : BGRepo.RepoReaderI, BGRepo.RepoWriterI
{
	...
	private static void ReadMetas(BGBinaryReader binder, BGRepo repo, bool multithreaded)
	{
		...
		bool lazyLoadEnabled = BGAddonLazyLoad.Enabled(repo);
		binder.ReadArray(() =>
		{
			BGMetaEntity meta = BGMetaEntity.FromBinary(binder, repo);
			int cursor = binder.ReadInt();
			if (lazyLoadEnabled)
			{
				BGBinaryReader metaReader = binder.NewReader(binder.Cursor);
				meta.LazyLoader = new BGLazyLoadMetaLoader(meta);
				meta.LazyLoader.AddAction(() =>
				{
					ReadMeta(metaReader, multithreaded, meta, null);
				});
				binder.SetCursor(cursor);
			}
			else
			{
				ReadMeta(binder, multithreaded, meta, multiThreadedLoader);
			}
		});
		binder.ReadArray(() =>
		{
			BGMetaView bGMetaView = BGMetaView.FromBinary(binder, repo);
		});
	...
	private static void ReadFieldValues(ArraySegment<byte> fieldValues, BGField field, int entitiesCount, Action<Exception> onError)
	{
		...
		if (constantSize > 0)
		{
			if (field is BGBinaryBulkLoaderStruct bGBinaryBulkLoaderStruct)
			{
				bGBinaryBulkLoaderStruct.FromBytes(new BGBinaryBulkRequestStruct(array, offset, entitiesCount));
				return;
			}
			for (int i = 0; i < entitiesCount; i++)
			{
				field.FromBytes(i, new ArraySegment<byte>(array, offset + i * constantSize, constantSize));
			}
			return;
		}
		...
		int num3 = num2 + num * 8;
		int num4 = 0;
		if (field is BGBinaryBulkLoaderClass bGBinaryBulkLoaderClass)
		{
			BGBinaryBulkRequestClass.CellRequest[] array2 = new BGBinaryBulkRequestClass.CellRequest[num];
			for (int j = 0; j < num; j++)
			{
				int num5 = num2 + j * 8;
				int entityIndex = (array[num5 + 3] << 24) | (array[num5 + 2] << 16) | (array[num5 + 1] << 8) | array[num5];
				int num6 = (array[num5 + 7] << 24) | (array[num5 + 6] << 16) | (array[num5 + 5] << 8) | array[num5 + 4];
				array2[j] = new BGBinaryBulkRequestClass.CellRequest(entityIndex, num3 + num4, num6 - num4);
				num4 = num6;
			}
			bGBinaryBulkLoaderClass.FromBytes(new BGBinaryBulkRequestClass(array, array2, onError));
			return;
		}
		for (int k = 0; k < num; k++)
		{
			int num7 = num2 + k * 8;
			int entityIndex2 = (array[num7 + 3] << 24) | (array[num7 + 2] << 16) | (array[num7 + 1] << 8) | array[num7];
			int num8 = (array[num7 + 7] << 24) | (array[num7 + 6] << 16) | (array[num7 + 5] << 8) | array[num7 + 4];
			try
			{
				field.FromBytes(entityIndex2, new ArraySegment<byte>(array, num3 + num4, num8 - num4));
			}
			catch (Exception obj)
			{
				onError(obj);
	...
	public byte[] Write(BGRepo repo)
	{
		...
		builder.AddArray(() =>
		{
			repo.ForEachMeta((BGMetaEntity meta) =>
			{
				...
				builder.ReplaceInt(builder.Count, count3);
			});
		}, repo.CountMeta);
		builder.AddArray(() =>
		{
