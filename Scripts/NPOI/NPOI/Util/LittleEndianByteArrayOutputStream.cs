...
public class LittleEndianByteArrayOutputStream : ILittleEndianOutput, IDelayableLittleEndianOutput
{
	...
	private void CheckPosition(int i)
	{
		...
		{
			throw new RuntimeException(string.Format(CultureInfo.InvariantCulture, "Buffer overrun i={0};endIndex={1};writeIndex={2}", new object[3] { i, _endIndex, _writeIndex }));
		}
	}
	...
	public void WriteInt(int v)
	{
		...
		_buf[writeIndex++] = (byte)(v & 0xFF);
		_buf[writeIndex++] = (byte)((v >> 8) & 0xFF);
		_buf[writeIndex++] = (byte)((v >> 16) & 0xFF);
		_buf[writeIndex++] = (byte)((v >> 24) & 0xFF);
	...
	{
		CheckPosition(size);
		LittleEndianByteArrayOutputStream result = new LittleEndianByteArrayOutputStream(_buf, _writeIndex, size);
		_writeIndex += size;
		return result;
	}
}
