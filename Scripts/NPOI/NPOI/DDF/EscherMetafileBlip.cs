...
public class EscherMetafileBlip : EscherBlipRecord
{
	...
	public void SetPictureData(byte[] pictureData)
	{
		...
		try
		{
			...
			deflaterOutputStream.Write(pictureData, 0, pictureData.Length);
			deflaterOutputStream.Close();
			raw_pictureData = memoryStream.ToArray();
		}
		catch (IOException ex)
		{
			throw new RuntimeException("Can't compress metafile picture data", ex);
		}
