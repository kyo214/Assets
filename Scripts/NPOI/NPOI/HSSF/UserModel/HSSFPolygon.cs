...
public class HSSFPolygon : HSSFSimpleShape
{
	...
	public int[] XPoints
	{
		get
		{
			...
			{
				short num = LittleEndian.GetShort(escherArrayProperty.GetElement(i), 0);
				array[i] = num;
			}
			return array;
		}
	...
	public int[] YPoints
	{
		get
		{
			...
			for (int i = 0; i < escherArrayProperty.NumberOfElementsInArray - 1; i++)
			{
				short num = LittleEndian.GetShort(escherArrayProperty.GetElement(i), 2);
				array[i] = num;
			}
			return array;
		}
	...
	protected override ObjRecord CreateObjRecord()
	{
		...
		{
			ObjectType = CommonObjectType.MicrosoftOfficeDrawing,
			IsLocked = true,
			IsPrintable = true,
			IsAutoFill = true,
			IsAutoline = true
	...
	public void SetPoints(int[] xPoints, int[] yPoints)
	{
		...
		escherArrayProperty2.SetElement(0, new byte[2] { 0, 64 });
		escherArrayProperty2.SetElement(1, new byte[2] { 0, 172 });
		for (int j = 0; j < xPoints.Length; j++)
		{
			escherArrayProperty2.SetElement(2 + j * 2, new byte[2] { 1, 0 });
			escherArrayProperty2.SetElement(3 + j * 2, new byte[2] { 0, 172 });
		}
		escherArrayProperty2.SetElement(escherArrayProperty2.NumberOfElementsInArray - 2, new byte[2] { 1, 96 });
		escherArrayProperty2.SetElement(escherArrayProperty2.NumberOfElementsInArray - 1, new byte[2] { 0, 128 });
