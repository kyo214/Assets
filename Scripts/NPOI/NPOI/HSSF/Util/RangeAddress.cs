...
public class RangeAddress
{
	...
	public int GetYPosition(string _subrange)
	{
		...
		if (_subrange.Length != 0)
		{
			...
			try
			{
				num = int.Parse(digitPart, CultureInfo.InvariantCulture);
				if (num > 66666)
				{
					num = -1;
				}
	...
	public int Get26Sys(string _s)
	{
		...
		if (!string.IsNullOrEmpty(_s))
		{
			...
			{
				int num4 = (int)(char.GetNumericValue(_s[num3]) - char.GetNumericValue('A') + 1.0);
				num += num4 * num2;
				num2 *= 26;
			}
			return num;
	...
	{
		return new StringBuilder(_source).Replace(_oldPattern, _newPattern).ToString();
	}
