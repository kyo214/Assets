...
public abstract class ChartRecordAggregate : RecordAggregate
{
	...
	protected void WriteStartBlock(RecordVisitor rv)
	{
		...
		if (IsInRule("AXES") && !blocks.Contains(ObjectKind.Axis))
		{
			...
			if (IsInRule("DVAXIS"))
			{
				startBlockRecord = ((GetContainer<DVAxisAggregate>("DVAXIS").Axis.AxisType != 0) ? StartBlockRecord.CreateStartBlock(ObjectKind.Axis, 0, 3) : StartBlockRecord.CreateStartBlock(ObjectKind.Axis, 0, 1));
				blocks.Push(startBlockRecord);
				rv.VisitRecord(startBlockRecord);
			}
		}
		...
		if (IsInRule("ATTACHEDLABEL") && !blocks.Contains(ObjectKind.AttachedLabelRecord))
		{
			if (IsInRule("DFTTEXT"))
			{
				DFTTextAggregate container5 = GetContainer<DFTTextAggregate>("DFTTEXT");
				startBlockRecord = ((!IsInRule("CRT") || container5.DefaultText.FormatType < TextFormatInfo.FontScaleNotSet) ? StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 2, (short)container5.DefaultText.FormatType) : StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 2, -1));
			}
			else
			{
				AttachedLabelAggregate container6 = GetContainer<AttachedLabelAggregate>("ATTACHEDLABEL");
				startBlockRecord = ((container6.ObjectLink.Link1 == 3) ? StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 4, 0) : ((container6.ObjectLink.Link1 == 2) ? StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 4, 1) : ((container6.ObjectLink.Link1 == 7) ? StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 4, 2) : ((!container6.IsFirst) ? StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 5, container6.ObjectLink.Link1, container6.ObjectLink.Link2) : StartBlockRecord.CreateStartBlock(ObjectKind.AttachedLabelRecord, 0)))));
			}
			blocks.Push(startBlockRecord);
			rv.VisitRecord(startBlockRecord);
		}
