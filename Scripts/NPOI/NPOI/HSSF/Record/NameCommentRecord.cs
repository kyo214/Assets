...
public class NameCommentRecord : StandardRecord
{
	...
	protected override int DataSize => 18 + (StringUtil.HasMultibyte(field_6_name_text) ? (field_6_name_text.Length * 2) : field_6_name_text.Length) + (StringUtil.HasMultibyte(field_7_comment_text) ? (field_7_comment_text.Length * 2) : field_7_comment_text.Length);
	...
	public override void Serialize(ILittleEndianOutput out1)
	{
		...
		bool flag = StringUtil.HasMultibyte(field_6_name_text);
		...
		bool flag2 = StringUtil.HasMultibyte(field_7_comment_text);
		out1.WriteByte(flag2 ? 1 : 0);
		if (flag2)
		{
	...
	public NameCommentRecord(RecordInputStream ris)
	{
		...
		if (((ILittleEndianInput)ris).ReadByte() == 0)
		{
			field_6_name_text = StringUtil.ReadCompressedUnicode(ris, nChars);
		}
		else
		{
			field_6_name_text = StringUtil.ReadUnicodeLE(ris, nChars);
		}
		if (((ILittleEndianInput)ris).ReadByte() == 0)
		{
		...
		{
			field_7_comment_text = StringUtil.ReadUnicodeLE(ris, nChars2);
		}
	}
