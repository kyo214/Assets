...
public class EscherAggregate : AbstractEscherHolderRecord
{
	...
	public override int RecordSize
	{
		...
		{
			int num = 0;
			List<EscherRecord> list = base.EscherRecords;
			int escherRecordSize = GetEscherRecordSize(list);
			byte[] data = new byte[escherRecordSize];
			List<int> list2 = new List<int>();
			int num2 = 0;
			foreach (EscherRecord item in list)
			{
				num2 += item.Serialize(num2, data, new RecordSizeListener(list2, item));
			}
			list2.Insert(0, 0);
			for (int i = 1; i < list2.Count; i++)
			{
				if (i == list2.Count - 1 && list2[i] < num2)
				{
					num += 4;
				}
				if (list2[i] - list2[i - 1] > 8224)
				{
					num += (list2[i] - list2[i - 1]) / 8224 * 4;
				}
			}
			int num3 = escherRecordSize + shapeToObj.Count * 4;
			if (escherRecordSize != 0 && list2.Count == 1)
			{
				num += 4;
			}
			int num4 = 0;
	...
	public static EscherAggregate CreateAggregate(List<RecordBase> records, int locFirstDrawingRecord)
	{
		...
		while (num + 1 < records.Count && IsDrawingLayerRecord(GetSid(records, num)))
		{
			try
			{
				...
				{
					byte[] recordData = ((DrawingRecord)records[num]).RecordData;
					memoryStream.Write(recordData, 0, recordData.Length);
				}
				else
				{
					byte[] data = ((ContinueRecord)records[num]).Data;
		...
		int num4 = num;
		records.RemoveRange(locFirstDrawingRecord, num4 - locFirstDrawingRecord);
	...
	{
		List<EscherRecord> list = base.EscherRecords;
		byte[] array = new byte[GetEscherRecordSize(list)];
		List<int> list2 = new List<int>();
		List<EscherRecord> list3 = new List<EscherRecord>();
		int num = 0;
		foreach (EscherRecord item in list)
		{
			num += item.Serialize(num, array, new SerializationListener(list2, list3, item));
		}
		list3.Insert(0, null);
		list2.Insert(0, 0);
		num = offset;
		int num2 = 0;
		int i;
		for (i = 1; i < list3.Count; i++)
		{
			int num3 = list2[i] - 1;
			int num4 = ((i != 1) ? list2[i - 1] : 0);
			...
			num2 += array2.Length;
			Record record = shapeToObj[list3[i]];
			num += record.Serialize(num, data);
			if (i == list3.Count - 1 && num3 < array.Length - 1)
			{
	...
	{
		RecordBase recordBase = records[loc];
		if (recordBase is Record)
		{
			return ((Record)recordBase).Sid;
		}
		return -1;
	}
	...
	{
		((EscherDgRecord)GetEscherContainer().GetChildById(-4088)).Options = (short)(dgId << 4);
	}

	internal void SetMainSpRecordId(int shapeId)
	{
		((EscherSpRecord)((EscherContainerRecord)((EscherContainerRecord)GetEscherContainer().GetChildById(-4093)).GetChild(0)).GetChildById(-4086)).ShapeId = shapeId;
	}
	...
	internal NoteRecord GetNoteRecordByObj(ObjRecord obj)
	{
		...
		if (!tailRec.ContainsKey(commonObjectDataSubRecord.ObjectId))
		{
			return null;
		}
