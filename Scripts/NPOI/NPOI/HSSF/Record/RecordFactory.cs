...
public class RecordFactory
{
	...
	private class ReflectionConstructorRecordCreator : I_RecordCreator
	{
		...
		{
			object[] parameters = new object[1] { in1 };
			try
			{
	...
	{
		NUM_RECORDS = 512;
		CONSTRUCTOR_ARGS = new Type[1] { typeof(RecordInputStream) };
		_recordCreatorsById = null;
		recordClasses = new Type[203]
		{
			typeof(ArrayRecord),
	...
	{
		I_RecordCreator i_RecordCreator = null;
		if (_recordCreatorsById.ContainsKey((short)sid))
		{
			i_RecordCreator = _recordCreatorsById[(short)sid];
		}
	...
	public static Record[] CreateRecord(RecordInputStream in1)
	{
		...
		{
			return new Record[1] { ConvertToNumberRecord((RKRecord)record) };
		}
		if (record is MulRKRecord)
		{
			return ConvertRKRecords((MulRKRecord)record);
		}
		return new Record[1] { record };
	}
	...
	public static Record CreateSingleRecord(RecordInputStream in1)
	{
		...
		{
			return _recordCreatorsById[in1.Sid].Create(in1);
		}
	...
	private static I_RecordCreator GetRecordCreator(Type recClass)
	{
		...
		try
		{
			return new ReflectionMethodRecordCreator(recClass.GetMethod("Create", CONSTRUCTOR_ARGS));
		}
		catch (Exception)
		{
