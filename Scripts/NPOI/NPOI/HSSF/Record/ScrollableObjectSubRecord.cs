...
public class ScrollableObjectSubRecord : SubRecord
{
	...
	public ScrollableObjectSubRecord(ILittleEndianInput in1, int size)
	{
		if (size != DataSize)
		{
			throw new RecordFormatException(string.Format(CultureInfo.CurrentCulture, "Expected size {0} but got ({1})", new object[2] { DataSize, size }));
		}
		in1.ReadInt();
	...
	{
		return new ScrollableObjectSubRecord
		{
			field_1_iVal = field_1_iVal,
			field_2_iMin = field_2_iMin,
			field_3_iMax = field_3_iMax,
			field_4_dInc = field_4_dInc,
			field_5_dPage = field_5_dPage,
			field_6_fHoriz = field_6_fHoriz,
			field_7_dxScroll = field_7_dxScroll,
			field_8_options = field_8_options
		};
	}
}
