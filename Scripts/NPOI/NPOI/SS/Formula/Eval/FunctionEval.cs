...
public sealed class FunctionEval
{
	...
	public static NPOI.SS.Formula.Functions.Function GetBasicFunction(int functionIndex)
	{
		...
		return functions[functionIndex] ?? throw new NotImplementedException("FuncIx=" + functionIndex);
	}
	...
	private static NPOI.SS.Formula.Functions.Function[] ProduceFunctions()
	{
		return new NPOI.SS.Formula.Functions.Function[368]
		{
			...
			new Lookup(),
			new NPOI.SS.Formula.Functions.Index(),
			new Rept(),
			TextFunction.MID,
			TextFunction.LEN,
			new Value(),
			...
			new DStarRunner(DStarRunner.DStarAlgorithmEnum.DMIN),
			...
			new DStarRunner(DStarRunner.DStarAlgorithmEnum.DGET),
