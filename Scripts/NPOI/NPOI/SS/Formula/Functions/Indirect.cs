...
public class Indirect : FreeRefFunction
{
	...
	public ValueEval Evaluate(ValueEval[] args, OperationEvaluationContext ec)
	{
		...
		try
		{
			text = OperandResolver.CoerceValueToString(OperandResolver.GetSingleValue(args[0], ec.RowIndex, ec.ColumnIndex));
			switch (args.Length)
			{
			case 1:
				isA1style = true;
	...
	private static ValueEval EvaluateIndirect(OperationEvaluationContext ec, string text, bool isA1style)
	{
		...
		if (Table.IsStructuredReference.Match(text2).Success)
		{
			Area3DPxg area3DPxg = null;
			try
			{
				area3DPxg = FormulaParser.ParseStructuredReference(text2, (IFormulaParsingWorkbook)ec.GetWorkbook(), ec.RowIndex);
			}
	...
	private static string[] ParseWorkbookAndSheetName(string text)
	{
		...
		case '\'':
		{
			...
			return new string[2] { text4, text5 };
		}
		case '[':
		{
