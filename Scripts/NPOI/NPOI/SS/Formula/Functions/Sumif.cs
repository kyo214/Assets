...
public class Sumif : Var2or3ArgFunction
{
	...
	{
		IMatchPredicate matchPredicate = Countif.CreateCriteriaPredicate(arg1, srcRowIndex, srcColumnIndex);
		...
		try
		{
			return new NumberEval(SumMatchingCells(aeRange, matchPredicate, aeSum));
		}
		catch (EvaluationException ex)
		{
			return ex.GetErrorEval();
