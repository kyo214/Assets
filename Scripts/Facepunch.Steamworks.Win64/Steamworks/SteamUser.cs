...
public class SteamUser : SteamClientClass<SteamUser>
{
	...
	{
		Dispatch.Install((SteamServersConnected_t x) =>
		{
			OnSteamServersConnected?.Invoke();
		});
		Dispatch.Install((SteamServerConnectFailure_t x) =>
		{
			OnSteamServerConnectFailure?.Invoke();
		});
		Dispatch.Install((SteamServersDisconnected_t x) =>
		{
			OnSteamServersDisconnected?.Invoke();
		});
		Dispatch.Install((ClientGameServerDeny_t x) =>
		{
			OnClientGameServerDeny?.Invoke();
		});
		Dispatch.Install((LicensesUpdated_t x) =>
		{
			OnLicensesUpdated?.Invoke();
		});
		...
		Dispatch.Install((GameWebCallback_t x) =>
		{
			OnGameWebCallback?.Invoke(x.URLUTF8());
		});
		Dispatch.Install((GetAuthSessionTicketResponse_t x) =>
		{
			OnGetAuthSessionTicketResponse?.Invoke(x);
		});
		Dispatch.Install((GetTicketForWebApiResponse_t x) =>
		{
			OnGetTicketForWebApiResponse?.Invoke(x);
		});
		Dispatch.Install((DurationControl_t x) =>
		{
			OnDurationControl?.Invoke(new DurationControl
			{
				_inner = x
			});
		});
