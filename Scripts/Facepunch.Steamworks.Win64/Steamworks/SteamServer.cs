...
public class SteamServer : SteamServerClass<SteamServer>
{
	...
	internal static void InstallEvents()
	{
		...
		{
			OnSteamServerConnectFailure?.Invoke(x.Result, x.StillRetrying);
		}, server: true);
		Dispatch.Install((SteamServersDisconnected_t x) =>
		{
			OnSteamServersDisconnected?.Invoke(x.Result);
		}, server: true);
		Dispatch.Install((SteamNetAuthenticationStatus_t x) =>
		{
			OnSteamNetAuthenticationStatus?.Invoke(x.Avail);
		}, server: true);
	}
