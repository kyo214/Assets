...
public class SteamUtils : SteamSharedClass<SteamUtils>
{
	...
	{
		Dispatch.Install((IPCountry_t x) =>
		{
			OnIpCountryChanged?.Invoke();
		}, server);
		Dispatch.Install((LowBatteryPower_t x) =>
		{
			OnLowBatteryPower?.Invoke(x.MinutesBatteryLeft);
		}, server);
		Dispatch.Install((SteamShutdown_t x) =>
		{
			SteamClosed();
		}, server);
		Dispatch.Install((GamepadTextInputDismissed_t x) =>
		{
			OnGamepadTextInputDismissed?.Invoke(x.Submitted);
		}, server);
	}
