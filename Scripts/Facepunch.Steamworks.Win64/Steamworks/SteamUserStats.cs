...
public class SteamUserStats : SteamClientClass<SteamUserStats>
{
	...
	internal static void InstallEvents()
	{
		...
		Dispatch.Install((UserStatsStored_t x) =>
		{
			OnUserStatsStored?.Invoke(x.Result);
		});
		Dispatch.Install((UserAchievementStored_t x) =>
		{
			OnAchievementProgress?.Invoke(new Achievement(x.AchievementNameUTF8()), (int)x.CurProgress, (int)x.MaxProgress);
		});
		Dispatch.Install((UserStatsUnloaded_t x) =>
		{
			OnUserStatsUnloaded?.Invoke(x.SteamIDUser);
		});
