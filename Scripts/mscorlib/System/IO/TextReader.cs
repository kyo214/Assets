...
public abstract class TextReader : MarshalByRefObject, IDisposable
{
	...
	public virtual async Task<string> ReadToEndAsync()
	{
		...
		{
			int charCount;
			while ((charCount = await ReadAsyncInternal(chars, default).ConfigureAwait(continueOnCapturedContext: false)) != 0)
			{
	...
	public virtual ValueTask<int> ReadAsync(Memory<char> buffer, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		return new ValueTask<int>(MemoryMarshal.TryGetArray((ReadOnlyMemory<char>)buffer, out segment) ? ReadAsync(segment.Array, segment.Offset, segment.Count) : Task<int>.Factory.StartNew((object state) =>
		{
	...
	internal virtual ValueTask<int> ReadAsyncInternal(Memory<char> buffer, CancellationToken cancellationToken)
	{
		...
		return new ValueTask<int>(Task<int>.Factory.StartNew((object obj) =>
		{
	...
	{
		ArraySegment<char> segment;
		return new ValueTask<int>(MemoryMarshal.TryGetArray((ReadOnlyMemory<char>)buffer, out segment) ? ReadBlockAsync(segment.Array, segment.Offset, segment.Count) : Task<int>.Factory.StartNew((object state) =>
		{
			Tuple<TextReader, Memory<char>> tuple = (Tuple<TextReader, Memory<char>>)state;
