...
public abstract class Stream : MarshalByRefObject, IDisposable, IAsyncDisposable
{
	...
	private void RunReadWriteTaskWhenReady(Task asyncWaiter, ReadWriteTask readWriteTask)
	{
		...
		asyncWaiter.ContinueWith((Task t, object state) =>
		{
	...
	private Task BeginEndWriteAsync(byte[] buffer, int offset, int count)
	{
		...
		return TaskFactory<VoidTaskResult>.FromAsyncTrim(this, new ReadWriteParameters
		{
			...
			Offset = offset,
			Count = count
		}, (Stream stream, ReadWriteParameters args, AsyncCallback callback, object state) => stream.BeginWrite(args.Buffer, args.Offset, args.Count, callback, state), (Stream stream, IAsyncResult asyncResult) =>
		{
