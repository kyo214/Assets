...
public static class Path
{
	...
	public static bool IsPathRooted(string path)
	{
		...
		return IsPathRooted((ReadOnlySpan<char>)path);
	}
	...
	private unsafe static string JoinInternal(ReadOnlySpan<char> first, ReadOnlySpan<char> second)
	{
		...
		fixed (char* reference = &MemoryMarshal.GetReference(first))
		{
			...
			{
				return string.Create(first.Length + second.Length + ((!flag) ? 1 : 0), new ValueTuple<IntPtr, int, IntPtr, int, bool>((IntPtr)reference, first.Length, (IntPtr)reference2, second.Length, flag), (Span<char> destination, [TupleElementNames(new string[] { "First", "FirstLength", "Second", "SecondLength", "HasSeparator" })] ValueTuple<IntPtr, int, IntPtr, int, bool> state) =>
				{
					new Span<char>((void*)state.Item1, state.Item2).CopyTo(destination);
					if (!state.Item5)
					{
						destination[state.Item2] = '\\';
					}
					new Span<char>((void*)state.Item3, state.Item4).CopyTo(destination.Slice(state.Item2 + ((!state.Item5) ? 1 : 0)));
				});
			}
		}
	}
	...
	private unsafe static string JoinInternal(ReadOnlySpan<char> first, ReadOnlySpan<char> second, ReadOnlySpan<char> third)
	{
		...
		fixed (char* reference = &MemoryMarshal.GetReference(first))
		{
			fixed (char* reference2 = &MemoryMarshal.GetReference(second))
			{
				...
				{
					return string.Create(first.Length + second.Length + third.Length + ((!flag) ? 1 : 0) + ((!flag2) ? 1 : 0), new ValueTuple<IntPtr, int, IntPtr, int, IntPtr, int, bool, ValueTuple<bool>>((IntPtr)reference, first.Length, (IntPtr)reference2, second.Length, (IntPtr)reference3, third.Length, flag, new ValueTuple<bool>(flag2)), (Span<char> destination, [TupleElementNames(new string[] { "First", "FirstLength", "Second", "SecondLength", "Third", "ThirdLength", "FirstHasSeparator", "ThirdHasSeparator", null })] ValueTuple<IntPtr, int, IntPtr, int, IntPtr, int, bool, ValueTuple<bool>> state) =>
					{
						new Span<char>((void*)state.Item1, state.Item2).CopyTo(destination);
						if (!state.Item7)
						{
						...
						new Span<char>((void*)state.Item3, state.Item4).CopyTo(destination.Slice(state.Item2 + ((!state.Item7) ? 1 : 0)));
						if (!state.Rest.Item1)
						{
							destination[destination.Length - state.Item6 - 1] = '\\';
						}
						new Span<char>((void*)state.Item5, state.Item6).CopyTo(destination.Slice(destination.Length - state.Item6));
					});
				}
			}
		}
		...
		fixed (char* reference = &MemoryMarshal.GetReference(first))
		{
			fixed (char* reference2 = &MemoryMarshal.GetReference(second))
			{
				fixed (char* reference3 = &MemoryMarshal.GetReference(third))
				{
					...
					{
						return string.Create(first.Length + second.Length + third.Length + fourth.Length + ((!flag) ? 1 : 0) + ((!flag2) ? 1 : 0) + ((!flag3) ? 1 : 0), new ValueTuple<IntPtr, int, IntPtr, int, IntPtr, int, IntPtr, ValueTuple<int, bool, bool, bool>>((IntPtr)reference, first.Length, (IntPtr)reference2, second.Length, (IntPtr)reference3, third.Length, (IntPtr)reference4, new ValueTuple<int, bool, bool, bool>(fourth.Length, flag, flag2, flag3)), (Span<char> destination, [TupleElementNames(new string[]
						{
						...
						})] ValueTuple<IntPtr, int, IntPtr, int, IntPtr, int, IntPtr, ValueTuple<int, bool, bool, bool>> state) =>
						{
							...
							new Span<char>((void*)state.Item3, state.Item4).CopyTo(destination.Slice(state.Item2 + ((!state.Rest.Item2) ? 1 : 0)));
							if (!state.Rest.Item3)
							{
								destination[state.Item2 + state.Item4 + ((!state.Rest.Item2) ? 1 : 0)] = '\\';
							}
							new Span<char>((void*)state.Item5, state.Item6).CopyTo(destination.Slice(state.Item2 + state.Item4 + ((!state.Rest.Item2) ? 1 : 0) + ((!state.Rest.Item3) ? 1 : 0)));
							if (!state.Rest.Item4)
							{
								destination[destination.Length - state.Rest.Item1 - 1] = '\\';
							}
							new Span<char>((void*)state.Item7, state.Rest.Item1).CopyTo(destination.Slice(destination.Length - state.Rest.Item1));
						});
					}
				}
			}
	...
	{
		return GetPathRoot(path.ToString());
	}
	...
	private static string GetRelativePath(string relativeTo, string path, StringComparison comparisonType)
	{
		...
		bool flag = PathInternal.EndsInDirectorySeparator(path);
	...
	public static bool IsPathFullyQualified(string path)
	{
		...
		return IsPathFullyQualified((ReadOnlySpan<char>)path);
	}
	...
	private static string CombineInternal(string first, string second)
	{
		...
		if (IsPathRooted((ReadOnlySpan<char>)second))
		{
			return second;
		}
