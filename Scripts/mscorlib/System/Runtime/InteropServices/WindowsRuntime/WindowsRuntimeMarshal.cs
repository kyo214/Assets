	...
	internal static class NativeOrStaticEventRegistrationImpl
	{
		...
		internal static void AddEventHandler<T>(Func<T, EventRegistrationToken> addMethod, Action<EventRegistrationToken> removeMethod, T handler)
		{
			...
			try
			{
				...
				try
				{
					ConditionalWeakTable<object, EventRegistrationTokenListWithCount> orCreateEventRegistrationTokenTable = GetOrCreateEventRegistrationTokenTable(instanceKey, removeMethod, out var tokenListCount);
		...
		internal static void RemoveEventHandler<T>(Action<EventRegistrationToken> removeMethod, T handler)
		{
			...
			try
			{
				ConditionalWeakTable<object, EventRegistrationTokenListWithCount> eventRegistrationTokenTableNoCreate = GetEventRegistrationTokenTableNoCreate(instanceKey, removeMethod, out var _);
				...
				{
					object key = eventRegistrationTokenTableNoCreate.FindEquivalentKeyUnsafe(handler, out var value);
					if (value == null)
					{
						return;
					}
