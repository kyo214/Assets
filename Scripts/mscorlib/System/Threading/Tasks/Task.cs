...
public class Task<TResult> : Task
{
	...
	internal Task(TResult result)
		: base(canceled: false, TaskCreationOptions.None, default)
	{
		m_result = result;
	}
...
public class Task : IThreadPoolWorkItem, IAsyncResult, IDisposable
{
	...
	public static Task CompletedTask { get; } = new Task(canceled: false, (TaskCreationOptions)16384, default);
	...
	internal void InternalRunSynchronously(TaskScheduler scheduler, bool waitForCompletion)
	{
		...
		if (MarkStarted())
		{
			...
			try
			{
				...
				{
					SpinThenBlockingWait(-1, default);
				}
				return;
			}
	...
	internal static Task<TResult> FromCancellation<TResult>(CancellationToken cancellationToken)
	{
		...
		return new Task<TResult>(canceled: true, default, TaskCreationOptions.None, cancellationToken);
	}
	...
	public static Task Delay(int millisecondsDelay, CancellationToken cancellationToken)
	{
		...
		if (cancellationToken.CanBeCanceled)
		{
			delayPromise.Registration = cancellationToken.InternalRegisterWithoutEC((object state) =>
			{
		...
		if (millisecondsDelay != -1)
		{
			delayPromise.Timer = new Timer((object state) =>
			{
				((DelayPromise)state).Complete();
	...
	private static Task<TResult[]> InternalWhenAll<TResult>(Task<TResult>[] tasks)
	{
		...
		return new Task<TResult[]>(canceled: false, Array.Empty<TResult>(), TaskCreationOptions.None, default);
	}
	...
	{
		return WhenAny((Task[])tasks).ContinueWith(Task<TResult>.TaskWhenAnyCast.Value, default, TaskContinuationOptions.DenyChildAttach | TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
	}

	public static Task<Task<TResult>> WhenAny<TResult>(IEnumerable<Task<TResult>> tasks)
	{
		return WhenAny((IEnumerable<Task>)tasks).ContinueWith(Task<TResult>.TaskWhenAnyCast.Value, default, TaskContinuationOptions.DenyChildAttach | TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
	}
