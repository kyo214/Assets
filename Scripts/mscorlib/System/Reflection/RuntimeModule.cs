...
internal class RuntimeModule : Module
{
	...
	{
		IntPtr intPtr = ResolveFieldToken(monoModule, metadataToken, ptrs_from_types(genericTypeArguments), ptrs_from_types(genericMethodArguments), out var error);
	...
	{
		MemberInfo memberInfo = ResolveMemberToken(monoModule, metadataToken, ptrs_from_types(genericTypeArguments), ptrs_from_types(genericMethodArguments), out var error);
		if (memberInfo == null)
		{
	...
	{
		IntPtr intPtr = ResolveMethodToken(monoModule, metadataToken, ptrs_from_types(genericTypeArguments), ptrs_from_types(genericMethodArguments), out var error);
		if (intPtr == IntPtr.Zero)
		{
	...
	{
		string text = ResolveStringToken(monoModule, metadataToken, out var error);
		if (text == null)
		{
	...
	{
		IntPtr intPtr = ResolveTypeToken(monoModule, metadataToken, ptrs_from_types(genericTypeArguments), ptrs_from_types(genericMethodArguments), out var error);
		if (intPtr == IntPtr.Zero)
		{
	...
	{
		byte[] array = ResolveSignature(monoModule, metadataToken, out var error);
		if (array == null)
		{
