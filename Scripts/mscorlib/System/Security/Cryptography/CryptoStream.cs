...
public class CryptoStream : Stream, IDisposable
{
	...
	{
		CheckReadArguments(buffer, offset, count);
		return ReadAsyncCore(buffer, offset, count, default, useAsync: false).GetAwaiter().GetResult();
	}
	...
	{
		CheckWriteArguments(buffer, offset, count);
		WriteAsyncCore(buffer, offset, count, default, useAsync: false).GetAwaiter().GetResult();
	}
