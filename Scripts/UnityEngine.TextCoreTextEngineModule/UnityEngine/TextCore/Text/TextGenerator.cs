...
internal class TextGenerator
{
	...
	private WordWrapState m_SavedEllipsisState = default;

	private WordWrapState m_SavedLastValidState = default;

	private WordWrapState m_SavedSoftLineBreakState = default;
	...
	private void GenerateTextMesh(TextGenerationSettings generationSettings, TextInfo textInfo)
	{
		...
		for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
		{
			...
			GlyphValueRecord glyphValueRecord = default;
			...
			textInfo.textElementInfo[m_CharacterCount].isVisible = false;
			bool flag13 = (m_LineJustification & (TextAlignment)0x10) == (TextAlignment)16 || (m_LineJustification & (TextAlignment)8) == (TextAlignment)8;
		...
		for (int k = 0; k < m_CharacterCount; k++)
		{
			...
			case TextAlignment.CaplineFlush:
			{
				...
				bool flag19 = (alignment & (TextAlignment)0x10) == (TextAlignment)16;
	...
	internal int SetArraySizes(TextProcessingElement[] textProcessingArray, TextGenerationSettings generationSettings, TextInfo textInfo)
	{
		...
		for (int i = 0; i < textProcessingArray.Length && textProcessingArray[i].unicode != 0; i++)
		{
			...
			TextElement textElement = GetTextElement(generationSettings, num2, m_CurrentFontAsset, m_FontStyleInternal, m_FontWeightInternal, out var isUsingAlternativeTypeface);
			if (textElement == null)
			{
	...
	protected void GetEllipsisSpecialCharacter(TextGenerationSettings generationSettings)
	{
		...
		TextSettings textSettings = generationSettings.textSettings;
		Character character = FontAssetUtilities.GetCharacterFromFontAsset(8230u, fontAsset, includeFallbacks: false, m_FontStyleInternal, m_FontWeightInternal, out var isAlternativeTypeface);
	...
	protected void GetUnderlineSpecialCharacter(TextGenerationSettings generationSettings)
	{
		...
		TextSettings textSettings = generationSettings.textSettings;
		Character characterFromFontAsset = FontAssetUtilities.GetCharacterFromFontAsset(95u, sourceFontAsset, includeFallbacks: false, m_FontStyleInternal, m_FontWeightInternal, out var _);
	...
	protected virtual Vector2 CalculatePreferredValues(ref float fontSize, Vector2 marginSize, bool isTextAutoSizingEnabled, TextWrappingMode textWrapMode, TextGenerationSettings generationSettings, TextInfo textInfo)
	{
		...
		for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
		{
			...
			bool flag10 = (m_LineJustification & (TextAlignment)0x10) == (TextAlignment)16 || (m_LineJustification & (TextAlignment)8) == (TextAlignment)8;
