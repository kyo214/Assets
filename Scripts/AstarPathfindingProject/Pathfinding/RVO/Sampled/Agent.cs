...
public class Agent : IAgent
{
	...
	private void GenerateObstacleVOs(VOBuffer vos)
	{
		...
		for (int i = 0; i < simulator.obstacles.Count; i++)
		{
			...
			do
			{
				...
				Vector2 vector = To2D(obstacleVertex2.position, out var elevation);
				Vector2 vector2 = To2D(obstacleVertex2.next.position, out var elevation2);
	...
	{
		Vector2 vector = Trace(vos, sampleAround1, out var score);
		if (DebugDraw)
		{
			Draw.Debug.CrossXZ(FromXZ(vector + position), Color.yellow, 0.5f);
		}
		Vector2 vector2 = Trace(vos, sampleAround2, out var score2);
		if (DebugDraw)
		{
	...
	private Vector2 EvaluateGradient(VOBuffer vos, Vector2 p, out float value)
	{
		...
		for (int i = 0; i < vos.length; i++)
		{
			Vector2 vector = vos.buffer[i].ScaledGradient(p, out var weight);
			if (weight > value)
			{
				value = weight;
				result = vector;
	...
	private Vector2 Trace(VOBuffer vos, Vector2 p, out float score)
	{
		...
		for (int i = 0; i < 50; i++)
		{
			...
			x = Sqr(x) * num;
			Vector2 vector = EvaluateGradient(vos, p, out var value);
			if (value < num2)
			{
				num2 = value;
				result = p;
