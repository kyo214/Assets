...
public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph, IRaycastableGraph
{
	...
	public virtual void RecalculateCell(int x, int z, bool resetPenalties = true, bool resetTags = true)
	{
		...
		Vector3 vector = collision.CheckHeight((Vector3)gridNodeBase.position, out var hit, out var walkable);
	...
	{
		hit = default;
		bool num = Linecast(from, to, out GridHitInfo hit2, trace, filter);
		hit.origin = from;
		hit.node = hit2.node;
		if (num)
		{
