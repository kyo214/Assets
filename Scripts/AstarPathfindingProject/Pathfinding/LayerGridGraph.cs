...
public class LayerGridGraph : GridGraph, IUpdatableGraph
{
	...
	{
		RaycastHit[] array = collision.CheckHeightAll(position, out var numHits2);
	...
	public override void RecalculateCell(int x, int z, bool resetPenalties = true, bool resetTags = true)
	{
		...
		HeightSample[] array = SampleHeights(collision, mergeSpanRange, base.transform.Transform(new Vector3((float)x + 0.5f, 0f, (float)z + 0.5f)), out var numHits);
