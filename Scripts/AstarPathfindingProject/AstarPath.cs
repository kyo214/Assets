...
public class AstarPath : VersionedMonoBehaviour
{
	...
	private void LogPathResults(Path path)
	{
		...
		{
			string text = ((IPathInternals)path).DebugString(logPathResults);
			...
			{
				UnityEngine.Debug.Log("Warning " + text);
			}
			else
			{
	...
	private void InitializePathProcessor()
	{
		...
		pathProcessor.OnPathPreSearch += (Path path) =>
		{
			OnPathPreSearch?.Invoke(path);
		};
		pathProcessor.OnPathPostSearch += (Path path) =>
		{
			LogPathResults(path);
	...
	public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
	{
		...
		NNInfoInternal internalInfo = default;
		int num2 = -1;
		if (array != null)
		{
