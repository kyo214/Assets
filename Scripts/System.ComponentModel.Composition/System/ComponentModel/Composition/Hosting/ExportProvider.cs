...
public abstract class ExportProvider
{
	...
	public IEnumerable<Export> GetExports(ImportDefinition definition, AtomicComposition atomicComposition)
	{
		...
		ExportCardinalityCheckResult exportCardinalityCheckResult = TryGetExportsCore(definition, atomicComposition, out var exports);
	...
	{
		EventHandler<ExportsChangeEventArgs> eventHandler = ExportsChanged;
	...
	{
		EventHandler<ExportsChangeEventArgs> eventHandler = ExportsChanging;
		if (eventHandler != null)
		{
			CompositionServices.TryFire(eventHandler, this, e).ThrowOnErrors(e.AtomicComposition);
		}
