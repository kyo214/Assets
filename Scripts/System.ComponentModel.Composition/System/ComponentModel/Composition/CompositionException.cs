...
public class CompositionException : Exception
{
		...
		: base(message, innerException)
	{
		...
		base.SerializeObjectState += (object exception, SafeSerializationEventArgs eventArgs) =>
		{
			CompositionExceptionData compositionExceptionData = default;
			if (_errors != null)
			{
	...
	private static IEnumerable<IEnumerable<CompositionError>> CalculatePaths(CompositionException exception)
	{
		...
		VisitCompositionException(exception, new VisitContext
		{
			...
			LeafVisitor = (Stack<CompositionError> path) =>
			{
				paths.Add(path.Copy());
			}
		});
