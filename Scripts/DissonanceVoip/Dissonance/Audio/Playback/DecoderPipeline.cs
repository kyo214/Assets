...
internal class DecoderPipeline : IDecoderPipeline, IVolumeProvider, IRemoteChannelProvider, IRateProvider
{
	...
	public TimeSpan BufferTime
	{
		get
		{
			long num = BufferCount;
			TimeSpan frameDuration = _frameDuration;
			return TimeSpan.FromTicks(num * frameDuration.Ticks);
		}
	}
	...
	{
		ConcurrentPool<byte[]> bytePool = _bytePool;
		ArraySegment<byte> encodedAudioFrame = packet.EncodedAudioFrame;
		bytePool.Put(encodedAudioFrame.Array);
	...
	public float Push(VoicePacket packet, DateTime now)
	{
		...
		DateTime value = _firstFrameArrival.Value;
		TimeSpan frameDuration = _frameDuration;
		DateTime dateTime = value + TimeSpan.FromTicks(frameDuration.Ticks * (packet.SequenceNumber - _firstFrameSeq));
