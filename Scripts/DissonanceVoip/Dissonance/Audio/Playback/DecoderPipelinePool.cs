...
internal static class DecoderPipelinePool
{
	...
	private static ConcurrentPool<DecoderPipeline> GetPool(FrameFormat format)
	{
		...
		{
			value = new ConcurrentPool<DecoderPipeline>(3, () => new DecoderPipeline(DecoderFactory.Create(format), id: _nextPipelineId.ToString(), inputFrameSize: format.FrameSize, completionHandler: (DecoderPipeline p) =>
			{
				p.Reset();
				Recycle(format, p);
			}));
