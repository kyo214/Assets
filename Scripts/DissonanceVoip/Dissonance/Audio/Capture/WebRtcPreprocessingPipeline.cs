...
internal class WebRtcPreprocessingPipeline : BasePreprocessingPipeline
{
	internal sealed class WebRtcPreprocessor : IDisposable
	{
		...
		private void SetFilterPreprocessor(IntPtr preprocessor)
		{
			...
			Bind((VoiceSettings s) => s.DenoiseAmount, "DenoiseAmount", (NoiseSuppressionLevels v) =>
			{
				NoiseSuppressionLevel = v;
			});
			Bind((VoiceSettings s) => s.AecSuppressionAmount, "AecSuppressionAmount", (AecSuppressionLevels v) =>
			{
				AecSuppressionLevel = v;
			});
			Bind((VoiceSettings s) => s.AecmRoutingMode, "AecmRoutingMode", (AecmRoutingMode v) =>
			{
				AecmSuppressionLevel = v;
			});
			Bind((VoiceSettings s) => s.VadSensitivity, "VadSensitivity", (VadSensitivityLevels v) =>
			{
		...
		private void Bind<T>(Func<VoiceSettings, T> getValue, string propertyName, Action<T> setValue)
		{
			...
			settings.PropertyChanged += (propertyChangedEventHandler = (object sender, PropertyChangedEventArgs args) =>
			{
	...
	internal sealed class RnnoisePreprocessor : IDisposable
	{
		...
		{
			_handle = new LockedValue<IntPtr>(IntPtr.Zero);
			Bind((VoiceSettings v) => v.BackgroundSoundRemovalEnabled, "BackgroundSoundRemovalEnabled", (bool a) =>
			{
				Enabled = a;
			});
			Bind((VoiceSettings v) => v.BackgroundSoundRemovalAmount, "BackgroundSoundRemovalAmount", (float a) =>
			{
				_removalAmount = a;
			});
		}
