...
internal struct PacketReader
{
	...
	public readonly ArraySegment<byte> Read
	{
		get
		{
			ArraySegment<byte> array = _array;
			byte[] array2 = array.Array;
			array = _array;
			return new ArraySegment<byte>(array2, array.Offset, _count);
		}
	}

	public readonly ArraySegment<byte> Unread
	{
		get
		{
			ArraySegment<byte> array = _array;
			byte[] array2 = array.Array;
			array = _array;
			int offset = array.Offset + _count;
			array = _array;
			return new ArraySegment<byte>(array2, offset, array.Count - _count);
		}
	}
	...
	private byte FastReadByte()
	{
		...
		byte[] array2 = array.Array;
		array = _array;
		return array2[array.Offset + _count - 1];
	}
	...
	public ArraySegment<byte> ReadByteSegment()
	{
		...
		Check(num, "byte[]");
		ArraySegment<byte> array = _array;
		ArraySegment<byte> result = new ArraySegment<byte>(array.Array, Unread.Offset, num);
		_count += num;
		return result;
	}
