...
internal struct PacketWriter
{
	...
	public readonly ArraySegment<byte> Written
	{
		get
		{
	...
	public PacketWriter Write([CanBeNull] string s)
	{
		...
		byte[] array2 = array.Array;
		array = _array;
		int bytes = uTF.GetBytes(s, 0, length, array2, array.Offset + _count + 2);
		if (bytes > 65535)
		{
	...
	public PacketWriter Write(ArraySegment<byte> data)
	{
		...
		ArraySegment<byte> source = data;
		ArraySegment<byte> array = _array;
		byte[] array2 = array.Array;
		array = _array;
		source.CopyToSegment(array2, array.Offset + _count);
		_count += data.Count;
		return this;
	}
