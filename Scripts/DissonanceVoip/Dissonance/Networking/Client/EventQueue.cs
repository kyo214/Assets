	...
	public bool DispatchEvents(DateTime? utcNow = null)
	{
		...
		for (int i = 0; i < value.Count; i++)
		{
			...
			case EventType.PlayerJoined:
				flag |= InvokeEvent(networkEvent.PlayerName, networkEvent.CodecSettings, PlayerJoined);
				break;
			case EventType.PlayerLeft:
				flag |= InvokeEvent(networkEvent.PlayerName, PlayerLeft);
				break;
			case EventType.PlayerStartedSpeaking:
				flag |= InvokeEvent(networkEvent.PlayerName, PlayerStartedSpeaking);
				break;
			case EventType.PlayerStoppedSpeaking:
				flag |= InvokeEvent(networkEvent.PlayerName, PlayerStoppedSpeaking);
				break;
			case EventType.VoiceData:
			{
				flag |= InvokeEvent(networkEvent.VoicePacket, VoicePacketReceived);
				...
				ArraySegment<byte> encodedAudioFrame = networkEvent.VoicePacket.EncodedAudioFrame;
