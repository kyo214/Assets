...
public class ProbeReferenceVolume
{
	...
	private ObjectPool<CellInfo> m_CellInfoPool = new ObjectPool<CellInfo>((CellInfo x) =>
	{
		x.Clear();
	}, null, collectionCheck: false);

	private ObjectPool<BlendingCellInfo> m_BlendingCellInfoPool = new ObjectPool<BlendingCellInfo>((BlendingCellInfo x) =>
	{
		x.Clear();
	}, null, collectionCheck: false);
	...
	{
		cellUpdateInfo = default;
		int numberOfBricksAtSubdiv = GetNumberOfBricksAtSubdiv(cell.position, cell.minSubdiv, out var minValidLocalIdxAtMaxRes, out var sizeOfValidIndicesAtMaxRes);
	...
	{
		ProbeBrickPool.BrickChunkAlloc item = default;
	...
	public void UpdateConstantBuffer(CommandBuffer cmd, ProbeVolumeShadingParameters parameters)
	{
		...
		ShaderVariablesProbeVolumes data = default;
