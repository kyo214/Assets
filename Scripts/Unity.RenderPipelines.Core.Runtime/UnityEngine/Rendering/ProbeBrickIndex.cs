...
internal class ProbeBrickIndex
{
	...
	private ObjectPool<BrickMeta> m_BrickMetaPool = new ObjectPool<BrickMeta>((BrickMeta x) =>
	{
		x.Clear();
	}, null, collectionCheck: false);

	private ObjectPool<List<VoxelMeta>> m_VoxelMetaListPool = new ObjectPool<List<VoxelMeta>>((List<VoxelMeta> x) =>
	{
		x.Clear();
	}, null, collectionCheck: false);

	private ObjectPool<VoxelMeta> m_VoxelMetaPool = new ObjectPool<VoxelMeta>((VoxelMeta x) =>
	{
		x.Clear();
	}, null, collectionCheck: false);
