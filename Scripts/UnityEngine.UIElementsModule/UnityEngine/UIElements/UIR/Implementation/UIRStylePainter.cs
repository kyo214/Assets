...
internal class UIRStylePainter : IStylePainter
{
	...
	public MeshWriteData AddGradientsEntry(int vertexCount, int indexCount, TextureId texture, Material material, MeshGenerationContext.MeshFlags flags)
	{
		...
		{
			pooledMeshWriteData.Reset(default, default);
			return pooledMeshWriteData;
		}
	...
	public void DrawBorder(MeshGenerationContextUtils.BorderParams borderParams)
	{
		...
		BuildEntryFromNativeMesh(meshData, null, default, isAtlas: false, null, MeshGenerationContext.MeshFlags.None, new Rect(0f, 0f, 1f, 1f), VertexFlags.IsSolid);
	}
	...
	public void DrawVisualElementBackground()
	{
		...
		MeshGenerationContextUtils.RectangleParams rectangleParams = default;
		...
		MeshGenerationContextUtils.RectangleParams rectParams2 = default;
		float num = visualElement.resolvedStyle.unitySliceScale;
		ScaleMode scaleMode = BackgroundPropertyHelper.ResolveUnityBackgroundScaleMode(computedStyle.backgroundPositionX, computedStyle.backgroundPositionY, computedStyle.backgroundRepeat, computedStyle.backgroundSize, out var valid);
	...
	private void DrawRectangleRepeat(MeshGenerationContextUtils.RectangleParams rectParams, Rect totalRect, float scaledPixelsPerPoint)
	{
		...
		RepeatRectUV item2 = default;
		RepeatRectUV item6 = default;
		RepeatRectUV item3 = default;
		RepeatRectUV item4 = default;
		RepeatRectUV item5 = default;
		RepeatRectUV item = default;
		for (int i = 0; i < 2; i++)
		{
