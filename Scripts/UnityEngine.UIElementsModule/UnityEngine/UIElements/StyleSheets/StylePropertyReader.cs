...
internal class StylePropertyReader
{
	...
	public FontDefinition ReadFontDefinition(int index)
	{
		...
		case StyleValueType.Keyword:
			if (stylePropertyValue.handle.valueIndex != 6)
			{
				...
				Debug.LogWarning("Invalid keyword for font " + valueIndex);
			}
			break;
		default:
	...
	public Background ReadBackground(int index)
	{
		...
		if (propertyValue.handle.valueType == StyleValueType.Keyword)
		{
			...
			{
				StyleValueKeyword valueIndex = (StyleValueKeyword)propertyValue.handle.valueIndex;
				Debug.LogWarning("Invalid keyword for image source " + valueIndex);
			}
		}
	...
	public static TransformOrigin ReadTransformOrigin(int valCount, StylePropertyValue val1, StylePropertyValue val2, StylePropertyValue zVvalue)
	{
		...
		case 1:
		{
			Length length = ReadTransformOriginEnum(val1, out var _, out var isHorizontal);
			if (isHorizontal)
			{
				x = length;
			}
		...
		case 2:
		{
			Length length2 = ReadTransformOriginEnum(val1, out var isVertical2, out var isHorizontal2);
			Length length3 = ReadTransformOriginEnum(val2, out var isVertical3, out var isHorizontal3);
	...
	{
		BackgroundRepeat result = default;
		if (valCount == 1)
		{
