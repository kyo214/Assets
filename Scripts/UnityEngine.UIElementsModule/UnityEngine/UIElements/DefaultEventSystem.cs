...
internal class DefaultEventSystem
{
	...
	private void SendIMGUIEvents()
	{
		...
		while (Event.PopEvent(m_Event))
		{
			...
			{
				Vector2 vector = UIElementsRuntimeUtility.MultiDisplayBottomLeftToPanelPosition(input.mousePosition, out var targetDisplay);
			...
			else if ((!m_SendingTouchEvents && !m_SendingPenEvent && m_Event.pointerType != UnityEngine.PointerType.Mouse) || m_Event.type == EventType.MouseEnterWindow || m_Event.type == EventType.MouseLeaveWindow)
			{
				...
				Vector3 mousePosition = UIElementsRuntimeUtility.MultiDisplayToLocalScreenPosition(m_Event.mousePosition, out var targetDisplay2);
				Vector2 delta2 = m_Event.delta;
				SendPositionBasedEvent(mousePosition, delta2, pointerId, targetDisplay2, (Vector3 panelPosition, Vector3 panelDelta, Event evt) =>
				{
	...
	private bool ProcessTouchEvents()
	{
		for (int i = 0; i < input.touchCount; i++)
		{
			...
			if (touch.type != TouchType.Indirect)
			{
				...
				SendPositionBasedEvent(touch.position, touch.deltaPosition, PointerId.touchPointerIdBase + touch.fingerId, targetDisplay, (Vector3 panelPosition, Vector3 panelDelta, Touch _touch) =>
				{
	...
	private bool ProcessPenEvents()
	{
		...
		SendPositionBasedEvent(lastPenContactEvent.position, lastPenContactEvent.deltaPos, PointerId.penPointerIdBase, null, (Vector3 panelPosition, Vector3 panelDelta, PenData _pen) =>
		{
