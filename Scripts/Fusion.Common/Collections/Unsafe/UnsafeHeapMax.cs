...
public struct UnsafeHeapMax
{
	...
	public unsafe static UnsafeHeapMax* Allocate<K, V>(int capacity, bool fixedSize = false) where K : unmanaged, IComparable<K> where V : unmanaged
	{
		return Allocate(capacity, sizeof(K), sizeof(V), fixedSize);
	}

	public unsafe static UnsafeHeapMax* Allocate(int capacity, int keyStride, int valStride, bool fixedSize = false)
	{
		capacity++;
	...
	public unsafe static void Free(UnsafeHeapMax* heap)
	{
		if (heap != null)
		{
	...
	public unsafe static int Capacity(UnsafeHeapMax* heap)
	{
		return heap->_items.Length - 1;
	}

	public unsafe static int Count(UnsafeHeapMax* heap)
	{
		return heap->_count - 1;
	}

	public unsafe static void Clear(UnsafeHeapMax* heap)
	{
		heap->_count = 1;
	}

	public unsafe static void Push<K, V>(UnsafeHeapMax* heap, K key, V val) where K : unmanaged, IComparable<K> where V : unmanaged
	{
	...
	public unsafe static void Pop<K, V>(UnsafeHeapMax* heap, out K key, out V val) where K : unmanaged, IComparable<K> where V : unmanaged
	{
		if (heap->_count <= 1)
		{
	...
	private unsafe static K Key<K>(UnsafeHeapMax* heap, int index) where K : unmanaged
	{
		return *(K*)UnsafeBuffer.Element(heap->_items.Ptr, index, heap->_items.Stride);
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private unsafe static void GetKeyVal<K, V>(UnsafeHeapMax* heap, int index, out K key, out V val) where K : unmanaged where V : unmanaged
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private unsafe static void SetKeyVal<K, V>(UnsafeHeapMax* heap, int index, K key, V val) where K : unmanaged where V : unmanaged
	{
		void* ptr = UnsafeBuffer.Element(heap->_items.Ptr, index, heap->_items.Stride);
		*(K*)ptr = key;
	...
	private unsafe static void ExpandHeap(UnsafeHeapMax* heap)
	{
	...
	public unsafe static UnsafeList.Iterator<T> GetIterator<T>(UnsafeHeapMax* heap) where T : unmanaged
	{
