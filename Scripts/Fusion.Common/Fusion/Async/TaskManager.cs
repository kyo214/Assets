...
public static class TaskManager
{
	...
	public static Task Service(Func<Task<bool>> recurringAction, CancellationToken cancellationToken, int interval, string serviceName = null)
	{
		...
		return TaskFactory.StartNew((Func<Task>)(async () =>
		{
			Log.Debug("Starting service: " + (serviceName ?? recurringAction.Method.Name));
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				{
					Log.Debug("Service canceled: " + (serviceName ?? recurringAction.Method.Name));
					throw;
				}
				catch (Exception msg)
				{
			...
			Log.Debug("Stopping service: " + (serviceName ?? recurringAction.Method.Name));
	...
	public static Task ContinueWhenAll(Task[] precedingTasks, Func<CancellationToken, Task> action, CancellationToken cancellationToken)
	{
		...
		return TaskFactory.ContinueWhenAll(precedingTasks, (Func<Task[], Task>)(async (Task[] tasks) =>
		{
			try
			{
				cancellationToken.ThrowIfCancellationRequested();
