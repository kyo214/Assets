...
public sealed class KCCNetworkModifiers : KCCNetworkProperty<KCCNetworkContext>
{
	private int _maxCount;

	public KCCNetworkModifiers(KCCNetworkContext context, int maxCount)
	...
	public unsafe override void Write(int* ptr)
	{
		...
		{
			KCCModifier kCCModifier = all[i];
			if (kCCModifier.NetworkID.IsValid)
			{
				KCCNetworkUtility.WriteNetworkID(ptr2, kCCModifier.NetworkID);
				ptr2 += 4;
				num++;
				if (num >= _maxCount)
				{
