...
public sealed class KCCNetworkVector2<TContext> : KCCNetworkProperty<TContext> where TContext : class
{
	...
	public KCCNetworkVector2(TContext context, float accuracy, Action<TContext, Vector2> set, Func<TContext, Vector2> get, Func<TContext, float, Vector2, Vector2, Vector2> interpolate)
		: base(context, 2)
	{
	...
	public unsafe override void Interpolate(InterpolationData interpolationData)
	{
		...
		Vector2 arg = ((_interpolate == null) ? Vector2.Lerp(vector, vector2, interpolationData.Alpha) : _interpolate(Context, interpolationData.Alpha, vector, vector2));
		_set(Context, arg);
	}
}
