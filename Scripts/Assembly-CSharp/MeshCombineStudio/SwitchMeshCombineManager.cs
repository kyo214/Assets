...
public class SwitchMeshCombineManager : MonoBehaviour
{
	...
	{
		t = base.transform;
		meshCombiners = GetComponentsInChildren<MeshCombiner>(includeInactive: true);
		...
		{
			gos[j] = t.GetChild(j).gameObject;
		}
		SetGosActive(active: false);
	...
	private void Update()
	{
		...
		{
			if (meshCombiners[0].combinedActive)
			{
				meshCombiners[0].SwapCombine();
			}
			SetGosActive(active: false);
