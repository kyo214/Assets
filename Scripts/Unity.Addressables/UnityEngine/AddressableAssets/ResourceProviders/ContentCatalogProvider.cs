			...
			public void LoadCatalogFromBundleAsync()
			{
				...
				if (ResourceManagerConfig.ShouldPathUseWebRequest(m_BundlePath))
				{
					...
					else
					{
						...
						webRequestQueueOperation.OnComplete = (Action<UnityWebRequestAsyncOperation>)Delegate.Combine(webRequestQueueOperation.OnComplete, (Action<UnityWebRequestAsyncOperation>)((UnityWebRequestAsyncOperation asyncOp) =>
						{
				...
				m_LoadBundleRequest.completed += (AsyncOperation loadOp) =>
				{
		...
		internal void LoadCatalog(string idToLoad, bool loadCatalogFromLocalBundle)
		{
			try
			{
				...
				if (loadCatalogFromLocalBundle)
				{
					...
					m_BundledCatalog.OnLoaded += (ContentCatalogData ccd) =>
					{
