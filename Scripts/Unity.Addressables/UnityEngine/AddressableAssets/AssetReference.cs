...
public class AssetReference : IKeyEvaluator
{
	...
	private static AsyncOperationHandle<T> CreateFailedOperation<T>()
	{
		...
		return Addressables.ResourceManager.CreateCompletedOperation<T>(default, new Exception("Attempting to load an asset reference that has no asset assigned to it.").Message);
	}
	...
	{
		AsyncOperationHandle<TObject> asyncOperationHandle = default;
		if (m_Operation.IsValid())
		{
	...
	{
		AsyncOperationHandle<SceneInstance> asyncOperationHandle = default;
		if (m_Operation.IsValid())
		{
