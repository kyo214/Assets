...
internal class AddressablesImpl : IEqualityComparer<IResourceLocation>
{
	...
	{
		handle.Completed += (AsyncOperationHandle<SceneInstance> sceneHandle) =>
		{
	...
	public AsyncOperationHandle<TObject> LoadAssetAsync<TObject>(object key)
	{
		...
		return ResourceManager.CreateCompletedOperationWithException<TObject>(default, new InvalidKeyException(key, type, this));
	}
	...
	public AsyncOperationHandle<IList<IResourceLocation>> LoadResourceLocationsAsync(IEnumerable keys, Addressables.MergeMode mode, Type type = null)
	{
		...
		return TrackHandle(ResourceManager.StartOperation(loadResourceLocationKeysOp, default));
	}
	...
	public AsyncOperationHandle<IList<IResourceLocation>> LoadResourceLocationsAsync(object key, Type type = null)
	{
		...
		return TrackHandle(ResourceManager.StartOperation(loadResourceLocationKeyOp, default));
	}
	...
	private AsyncOperationHandle<long> ComputeCatalogSizeWithChain(IResourceLocation catalogLoc)
	{
		...
		return ResourceManager.CreateChainOperation(dependentOp, (AsyncOperationHandle op) =>
		{
			try
			{
	...
	internal AsyncOperationHandle<long> GetRemoteCatalogHeaderSize(IResourceLocation catalogLoc)
	{
		...
		return ResourceManager.CreateChainOperation(ResourceManager.StartOperation(operation, default), (AsyncOperationHandle<UnityWebRequest> getOp) =>
		{
	...
	{
		handle.CompletedTypeless += (AsyncOperationHandle op) =>
		{
			op.Release();
		};
	}
	...
	public AsyncOperationHandle<SceneInstance> LoadSceneAsync(object key, LoadSceneParameters loadSceneParameters, bool activateOnLoad = true, int priority = 100, bool trackHandle = true)
	{
		...
		{
			return ResourceManager.CreateCompletedOperationWithException<SceneInstance>(default, new InvalidKeyException(key, typeof(SceneInstance), this));
		}
