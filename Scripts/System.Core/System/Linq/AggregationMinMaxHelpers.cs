...
internal static class AggregationMinMaxHelpers<T>
{
	private static T Reduce(IEnumerable<T> source, int sign)
	{
		...
		return new AssociativeAggregationOperator<T, Pair<bool, T>, T>(source, new Pair<bool, T>(first: false, default), null, seedIsSpecified: true, intermediateReduce, finalReduce, resultSelector, default(T) != null, QueryAggregationOptions.AssociativeCommutative).Aggregate();
	}
