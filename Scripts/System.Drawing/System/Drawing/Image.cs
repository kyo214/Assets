...
public abstract class Image : MarshalByRefObject, IDisposable, ICloneable, ISerializable
{
	...
	public PropertyItem[] PropertyItems
	{
		get
		{
			GdipPropertyItem structure = default;
	...
	internal Image(SerializationInfo info, StreamingContext context)
	{
		...
		{
			if (string.Compare(item.Name, "Data", ignoreCase: true) != 0)
			{
				continue;
			}
	...
	public static Image FromFile(string filename, bool useEmbeddedColorManagement)
	{
		...
		Status status = ((!useEmbeddedColorManagement) ? GDIPlus.GdipLoadImageFromFile(filename, out var image) : GDIPlus.GdipLoadImageFromFileICM(filename, out image));
