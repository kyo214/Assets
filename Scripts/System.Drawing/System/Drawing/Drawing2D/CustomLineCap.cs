...
public class CustomLineCap : MarshalByRefObject, ICloneable, IDisposable
{
	...
	public LineJoin StrokeJoin
	{
		get
		{
			int num = GDIPlus.GdipGetCustomLineCapStrokeJoin(new HandleRef(this, nativeCap), out var lineJoin);
			if (num != 0)
			{
	...
	public LineCap BaseCap
	{
		get
		{
			int num = GDIPlus.GdipGetCustomLineCapBaseCap(new HandleRef(this, nativeCap), out var baseCap);
			if (num != 0)
			{
				throw SafeNativeMethods.Gdip.StatusException(num);
			}
	...
	public float BaseInset
	{
		get
		{
			int num = GDIPlus.GdipGetCustomLineCapBaseInset(new HandleRef(this, nativeCap), out var inset);
			if (num != 0)
			{
				throw SafeNativeMethods.Gdip.StatusException(num);
			}
	...
	public float WidthScale
	{
		get
		{
			int num = GDIPlus.GdipGetCustomLineCapWidthScale(new HandleRef(this, nativeCap), out var widthScale);
			if (num != 0)
			{
				throw SafeNativeMethods.Gdip.StatusException(num);
			}
	...
	{
		int num = GDIPlus.GdipCreateCustomLineCap(new HandleRef(fillPath, fillPath?.nativePath ?? IntPtr.Zero), new HandleRef(strokePath, strokePath?.nativePath ?? IntPtr.Zero), baseCap, baseInset, out var customCap);
		if (num != 0)
		{
			throw SafeNativeMethods.Gdip.StatusException(num);
		}
	...
	{
		int num = GDIPlus.GdipCloneCustomLineCap(new HandleRef(this, nativeCap), out var clonedCap);
		if (num != 0)
		{
			throw SafeNativeMethods.Gdip.StatusException(num);
		}
