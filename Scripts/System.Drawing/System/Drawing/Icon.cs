...
public sealed class Icon : MarshalByRefObject, ISerializable, ICloneable, IDisposable
{
	...
	private Icon(SerializationInfo info, StreamingContext context)
	{
		...
		{
			if (string.Compare(item.Name, "IconData", ignoreCase: true) == 0)
			{
				memoryStream = new MemoryStream((byte[])item.Value);
			}
			if (string.Compare(item.Name, "IconSize", ignoreCase: true) == 0)
			{
	...
	private void InitFromStreamWithSize(Stream stream, int width, int height)
	{
		...
		bool flag = false;
		IconDirEntry iconDirEntry = default;
		for (int i = 0; i < num; i++)
		{
		...
		for (int l = 0; l < num; l++)
		{
			...
			IconImage iconImage = new IconImage();
			BitmapInfoHeader iconHeader = default;
			stream.Seek(iconDir.idEntries[l].imageOffset, SeekOrigin.Begin);
