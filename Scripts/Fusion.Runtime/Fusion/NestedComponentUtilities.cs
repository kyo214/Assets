...
public static class NestedComponentUtilities
{
	...
	public static T EnsureRootComponentExists<T, StopOnT>(this Transform transform) where T : Component where StopOnT : Component
	{
		StopOnT parentComponent = transform.GetParentComponent<StopOnT>();
	...
	public static T GetNestedComponentInChildren<T, StopOnT>(this Transform t, bool includeInactive) where T : class where StopOnT : class
	{
		if (t.TryGetComponent<T>(out var component))
		{
			return component;
		...
		while (nodesQueue.Count > 0)
		{
			...
			for (int childCount = transform.childCount; i < childCount; i++)
			{
				...
				if ((includeInactive || child.gameObject.activeSelf) && !child.TryGetComponent<StopOnT>(out var _))
				{
	...
	public static T GetNestedComponentInParent<T, StopOnT>(this Transform t) where T : class where StopOnT : class
	{
		...
		do
		{
			...
			if (transform.TryGetComponent<StopOnT>(out var _))
			{
				return null;
			}
	...
	public static T GetNestedComponentInParents<T, StopOnT>(this Transform t) where T : class where StopOnT : class
	{
		if (t.TryGetComponent<T>(out var component))
		{
			return component;
		...
		while ((bool)parent)
		{
			...
			if (parent.TryGetComponent<StopOnT>(out var _))
			{
				return null;
			}
			parent = parent.parent;
	...
	public static void GetNestedComponentsInParents<T, StopT>(this Transform t, List<T> list) where T : class where StopT : class
	{
		t.GetComponents(list);
		if (t.TryGetComponent<StopT>(out var component))
		{
			return;
		}
		Transform parent = t.parent;
		...
		do
		{
			...
			if (parent.TryGetComponent<StopT>(out component))
			{
				break;
			}
			parent = parent.parent;
	...
	public static List<T> GetNestedComponentsInChildren<T, StopOnT>(this Transform t, List<T> list, bool includeInactive = true) where T : class where StopOnT : class
	{
		nodesQueue.Clear();
		if (list == null)
		{
		...
		for (int childCount = t.childCount; i < childCount; i++)
		{
			...
			if ((includeInactive || child.gameObject.activeSelf) && !child.TryGetComponent<StopOnT>(out component))
			{
				nodesQueue.Enqueue(child);
			}
		}
		...
		try
		{
			while (nodesQueue.Count > 0)
			{
				...
				for (int childCount2 = transform.childCount; j < childCount2; j++)
				{
					...
					if ((includeInactive || child2.gameObject.activeSelf) && !child2.TryGetComponent<StopOnT>(out component))
					{
	...
	public static void GetNestedComponentsInChildren<T, SearchT, StopT>(this Transform t, bool includeInactive, List<T> list) where T : class where SearchT : class
	{
		list.Clear();
		...
		List<SearchT> list2 = RecyclableList<SearchT>.List;
		nodesQueue.Clear();
		nodesQueue.Enqueue(t);
		try
		{
			while (nodesQueue.Count > 0)
			{
				...
				{
					Transform child = transform.GetChild(i);
					if ((includeInactive || child.gameObject.activeSelf) && child.GetComponent<StopT>() == null)
					{
						nodesQueue.Enqueue(child);
					}
				}
	...
	public static CastT[] FindObjectsOfTypeInOrder<T, CastT>(this Scene scene, bool includeInactive = false) where T : class where CastT : class
	{
		List<CastT> list = new List<CastT>();
		GameObject[] rootGameObjects = scene.GetRootGameObjects();
		GameObject[] array = rootGameObjects;
		foreach (GameObject gameObject in array)
		{
	...
	public static void FindObjectsOfTypeInOrder<T, CastT>(this Scene scene, List<CastT> list, bool includeInactive = false) where T : class where CastT : class
	{
		list.Clear();
		GameObject[] rootGameObjects = scene.GetRootGameObjects();
		GameObject[] array = rootGameObjects;
	...
	[Obsolete("Use FindObjectsOfTypeInOrder instead.")]
	public static T[] FindObjectOfTypeInOrder<T>(this Scene scene) where T : class
	{
		List<T> list = new List<T>();
		GameObject[] rootGameObjects = scene.GetRootGameObjects();
		GameObject[] array = rootGameObjects;
	...
	[Obsolete("Use FindObjectsOfTypeInOrder instead.")]
	public static CastT[] FindObjectOfTypeInOrder<T, CastT>(this Scene scene) where T : class where CastT : class
	{
		List<CastT> list = new List<CastT>();
		GameObject[] rootGameObjects = scene.GetRootGameObjects();
		GameObject[] array = rootGameObjects;
