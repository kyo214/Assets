...
public struct NetworkDictionary<K, V> : IEnumerable<KeyValuePair<K, V>>, IEnumerable
{
	...
	{
		Assert.Check(Primes.IsPrime(capacity), "Capacity not prime", capacity);
	...
	public unsafe NetworkDictionary<K, V> Remap(void* dictionary)
	{
		Assert.Check(_data);
		Assert.Check(_capacity > 0);
		Assert.Check(_keyReaderWriter);
		Assert.Check(_valReaderWriter);
		Assert.Check(dictionary);
		NetworkDictionary<K, V> result = this;
		result._data = (int*)dictionary;
		return result;
	}
