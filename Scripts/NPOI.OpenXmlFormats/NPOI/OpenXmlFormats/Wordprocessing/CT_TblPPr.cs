...
public class CT_TblPPr
{
	...
	{
		sw.Write($"<w:{nodeName}");
		XmlHelper.WriteAttribute(sw, "w:leftFromText", leftFromText);
		XmlHelper.WriteAttribute(sw, "w:rightFromText", rightFromText);
		XmlHelper.WriteAttribute(sw, "w:topFromText", topFromText);
		XmlHelper.WriteAttribute(sw, "w:bottomFromText", bottomFromText);
