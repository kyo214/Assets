...
public class CT_P
{
	...
	public static CT_P Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_P.paraIdField = XmlHelper.ReadBytes(node.Attributes["w14:paraId"]);
		}
		if (node.Attributes["w14:textId"] != null)
		{
			cT_P.textIdField = XmlHelper.ReadBytes(node.Attributes["w14:textId"]);
		}
	...
	{
		sw.Write($"<w:{nodeName}");
		XmlHelper.WriteAttribute(sw, "w14:paraId", paraIdField);
		XmlHelper.WriteAttribute(sw, "w14:textId", textIdField);
	...
	private T InsertNewObject<T>(ParagraphItemsChoiceType type, int p) where T : class, new()
	{
		...
		lock (this)
		{
			int num = GetObjectIndex(type, p);
			if (num == -1)
			{
				num = 0;
			}
			itemsElementNameField.Insert(num, type);
			itemsField.Insert(num, val);
			return val;
		}
	}
