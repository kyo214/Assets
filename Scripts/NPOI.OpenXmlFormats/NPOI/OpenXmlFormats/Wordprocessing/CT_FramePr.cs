...
public class CT_FramePr
{
	...
	public static CT_FramePr Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_FramePr.anchorLock = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:anchorLock"].Value, ignoreCase: true);
		}
		return cT_FramePr;
	}
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		XmlHelper.WriteAttribute(sw, "w:h", h);
		XmlHelper.WriteAttribute(sw, "w:vSpace", vSpace);
		XmlHelper.WriteAttribute(sw, "w:hSpace", hSpace);
