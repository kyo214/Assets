...
public class CT_R
{
	...
	public static CT_R Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		foreach (XmlNode childNode in node.ChildNodes)
		{
			...
			{
				cT_R.alternateContent = CT_AlternateContent.Parse(childNode, namespaceManager);
			}
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		foreach (object item in Items)
		{
			...
			{
				((CT_Markup)item).Write(sw, "commentReference");
			}
			...
			else if (item is CT_FtnEdnRef && ItemsElementName[num] == RunItemsChoiceType.endnoteReference)
			{
				((CT_FtnEdnRef)item).Write(sw, "endnoteReference");
			}
			else if (item is CT_FtnEdnRef && ItemsElementName[num] == RunItemsChoiceType.footnoteReference)
			{
