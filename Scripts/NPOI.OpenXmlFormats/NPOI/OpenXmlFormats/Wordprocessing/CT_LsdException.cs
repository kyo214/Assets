...
public class CT_LsdException
{
	...
	public static CT_LsdException Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			ex.locked = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:locked"].Value, ignoreCase: true);
		}
		ex.uiPriority = XmlHelper.ReadString(node.Attributes["w:uiPriority"]);
		if (node.Attributes["w:semiHidden"] != null)
		{
			ex.semiHidden = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:semiHidden"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:unhideWhenUsed"] != null)
		{
			ex.unhideWhenUsed = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:unhideWhenUsed"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:qFormat"] != null)
		{
			ex.qFormat = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:qFormat"].Value, ignoreCase: true);
		}
		return ex;
	}
