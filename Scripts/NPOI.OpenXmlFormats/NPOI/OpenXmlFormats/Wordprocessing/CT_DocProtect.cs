...
public class CT_DocProtect
{
	...
	public static CT_DocProtect Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_DocProtect.formatting = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:formatting"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:enforcement"] != null)
		{
			cT_DocProtect.enforcement = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:enforcement"].Value, ignoreCase: true);
		}
