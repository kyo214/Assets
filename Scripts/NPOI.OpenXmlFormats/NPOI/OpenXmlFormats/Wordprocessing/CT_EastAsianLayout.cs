...
public class CT_EastAsianLayout
{
	...
	public static CT_EastAsianLayout Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_EastAsianLayout.combine = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:combine"].Value, ignoreCase: true);
		}
		...
		{
			cT_EastAsianLayout.vert = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:vert"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:vertCompress"] != null)
		{
			cT_EastAsianLayout.vertCompress = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:vertCompress"].Value, ignoreCase: true);
		}
