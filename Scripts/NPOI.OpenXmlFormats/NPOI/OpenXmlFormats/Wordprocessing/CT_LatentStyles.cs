...
public class CT_LatentStyles
{
	...
	public static CT_LatentStyles Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_LatentStyles.defLockedState = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:defLockedState"].Value, ignoreCase: true);
		}
		...
		{
			cT_LatentStyles.defSemiHidden = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:defSemiHidden"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:defUnhideWhenUsed"] != null)
		{
			cT_LatentStyles.defUnhideWhenUsed = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:defUnhideWhenUsed"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:defQFormat"] != null)
		{
			cT_LatentStyles.defQFormat = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:defQFormat"].Value, ignoreCase: true);
		}
	...
	{
		sw.Write($"<w:{nodeName}");
		XmlHelper.WriteAttribute(sw, "w:defLockedState", (defLockedState == ST_OnOff.off) ? "0" : "1");
		XmlHelper.WriteAttribute(sw, "w:defUIPriority", defUIPriority);
		XmlHelper.WriteAttribute(sw, "w:defSemiHidden", (defSemiHidden == ST_OnOff.off) ? "0" : "1");
		XmlHelper.WriteAttribute(sw, "w:defUnhideWhenUsed", (defUnhideWhenUsed == ST_OnOff.off) ? "0" : "1");
		XmlHelper.WriteAttribute(sw, "w:defQFormat", (defQFormat == ST_OnOff.off) ? "0" : "1");
	...
	public int SizeOfLsdExceptionArray()
	{
		if (lsdExceptionField != null)
		{
			return lsdExceptionField.Count;
		}
		return 0;
	}
}
