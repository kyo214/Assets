...
public class CT_TblPr : CT_TblPrBase
{
	...
	public new static CT_TblPr Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		foreach (XmlNode childNode in node.ChildNodes)
		{
			...
			{
				cT_TblPr.tblCaption = CT_String.Parse(childNode, namespaceManager);
			}
			else if (childNode.LocalName == "tblDescription")
			{
				cT_TblPr.tblDescription = CT_String.Parse(childNode, namespaceManager);
			}
		}
		return cT_TblPr;
	}
	...
	internal new void Write(StreamWriter sw, string nodeName)
	{
		...
		if (base.tblCaption != null)
		{
			base.tblCaption.Write(sw, "tblCaption");
		}
		if (base.tblDescription != null)
		{
			base.tblDescription.Write(sw, "tblDescription");
		}
		sw.Write($"</w:{nodeName}>");
	}
