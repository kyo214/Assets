...
public class CT_WritingStyle
{
	...
	public static CT_WritingStyle Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_WritingStyle.nlCheck = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:nlCheck"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:checkStyle"] != null)
		{
			cT_WritingStyle.checkStyle = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:checkStyle"].Value, ignoreCase: true);
		}
