...
public class CT_TrackChangesView
{
	...
	public static CT_TrackChangesView Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_TrackChangesView.markup = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:markup"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:comments"] != null)
		{
			cT_TrackChangesView.comments = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:comments"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:insDel"] != null)
		{
			cT_TrackChangesView.insDel = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:insDel"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:formatting"] != null)
		{
			cT_TrackChangesView.formatting = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:formatting"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:inkAnnotations"] != null)
		{
			cT_TrackChangesView.inkAnnotations = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:inkAnnotations"].Value, ignoreCase: true);
		}
