...
public class CT_SimpleField
{
	...
	public static CT_SimpleField Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_SimpleField.fldLock = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:fldLock"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:dirty"] != null)
		{
			cT_SimpleField.dirty = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:dirty"].Value, ignoreCase: true);
		}
