...
public class CT_Style
{
	...
	public static CT_Style Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_Style.@default = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:default"].Value, ignoreCase: true);
		}
		if (node.Attributes["w:customStyle"] != null)
		{
			cT_Style.customStyle = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:customStyle"].Value, ignoreCase: true);
		}
