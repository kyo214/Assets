...
public class CT_DataBinding
{
	...
	public static CT_DataBinding Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		return new CT_DataBinding
		{
			prefixMappings = XmlHelper.ReadString(node.Attributes["w:prefixMappings"]),
			xpath = XmlHelper.ReadString(node.Attributes["w:xpath"]),
			storeItemID = XmlHelper.ReadString(node.Attributes["w:storeItemID"])
		};
	}
