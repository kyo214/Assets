...
public class CT_Caption
{
	...
	public static CT_Caption Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_Caption.chapNum = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:chapNum"].Value, ignoreCase: true);
		}
		cT_Caption.heading = XmlHelper.ReadString(node.Attributes["w:heading"]);
		if (node.Attributes["w:noLabel"] != null)
		{
			cT_Caption.noLabel = (ST_OnOff)Enum.Parse(typeof(ST_OnOff), node.Attributes["w:noLabel"].Value, ignoreCase: true);
		}
