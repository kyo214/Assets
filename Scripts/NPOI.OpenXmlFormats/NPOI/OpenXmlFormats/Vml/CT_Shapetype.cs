...
public class CT_Shapetype
{
	...
	public static CT_Shapetype Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		{
			cT_Shapetype.id = NPOI.OpenXml4Net.Util.XmlHelper.ReadString(node.Attributes["o:spt"]);
		}
	...
	{
		sw.Write($"<v:{nodeName}");
		NPOI.OpenXmlFormats.Util.XmlHelper.WriteAttribute(sw, "stroked", stroked, defaultValue: true);
		NPOI.OpenXmlFormats.Util.XmlHelper.WriteAttribute(sw, "filled", filled, defaultValue: true);
		...
		NPOI.OpenXml4Net.Util.XmlHelper.WriteAttribute(sw, "o:spt", spt);
		...
		if (textpath != null)
		{
			foreach (CT_TextPath item in textpath)
			{
				item.Write(sw, "textpath");
			}
		}
		if (formulas != null)
		{
		...
		if (shadow != null)
		{
			foreach (CT_Shadow item3 in shadow)
			{
				item3.Write(sw, "shadow");
			}
		}
		if (@lock != null)
		{
		...
		if (textbox != null)
		{
			foreach (CT_Textbox item4 in textbox)
			{
				item4.Write(sw, "textbox");
			}
		}
		if (imagedata != null)
		{
