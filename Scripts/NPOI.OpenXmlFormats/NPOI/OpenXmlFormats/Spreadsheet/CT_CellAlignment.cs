...
public class CT_CellAlignment
{
	...
	{
		sw.Write($"<{nodeName}");
		if (horizontal != ST_HorizontalAlignment.general)
		{
		...
		XmlHelper.WriteAttribute(sw, "textRotation", textRotation);
		if (wrapText)
		{
		...
		XmlHelper.WriteAttribute(sw, "readingOrder", readingOrder);
		sw.Write("/>");
	}
