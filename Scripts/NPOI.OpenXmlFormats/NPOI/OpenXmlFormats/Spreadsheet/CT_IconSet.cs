...
public class CT_IconSet
{
	...
	public static CT_IconSet Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_IconSet.showValue = XmlHelper.ReadBool(node.Attributes["showValue"], blankValue: true);
		if (!cT_IconSet.showValue)
		{
			cT_IconSet.showValueFieldSpecified = true;
		}
		cT_IconSet.percent = XmlHelper.ReadBool(node.Attributes["percent"], blankValue: true);
	...
	public CT_Cfvo AddNewCfvo()
	{
		...
		cfvoField.Add(cT_Cfvo);
		return cT_Cfvo;
	}

	public bool IsSetShowValue()
	{
		return showValueFieldSpecified;
	}
