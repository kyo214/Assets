...
public class CT_CfRule
{
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		if (dxfIdSpecified)
		{
			XmlHelper.WriteAttribute(sw, "dxfId", dxfId, writeIfBlank: true);
		}
		XmlHelper.WriteAttribute(sw, "priority", priority, writeIfBlank: true);
		if (stopIfTrue)
		{
		...
		if (aboveAverage)
		{
			XmlHelper.WriteAttribute(sw, "aboveAverage", aboveAverage, writeIfBlank: true);
		}
		if (percent)
		{
		...
		if (formula != null)
		{
			...
			{
				sw.Write($"<formula>{XmlHelper.EncodeXml(item)}</formula>");
			}
		}
		sw.Write($"</{nodeName}>");
	}
	...
	{
		return iconSet = new CT_IconSet();
	}

	public bool IsSetIconSet()
	{
		return iconSetField != null;
	}
