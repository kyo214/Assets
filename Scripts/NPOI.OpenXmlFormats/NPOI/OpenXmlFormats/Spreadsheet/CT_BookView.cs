...
public class CT_BookView
{
	...
	[XmlIgnore]
	public bool extLstSpecified => extLstField != null;
	...
	public static CT_BookView Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_BookView.showHorizontalScroll = XmlHelper.ReadBool(node.Attributes["showHorizontalScroll"], blankValue: true);
		cT_BookView.showVerticalScroll = XmlHelper.ReadBool(node.Attributes["showVerticalScroll"], blankValue: true);
		cT_BookView.showSheetTabs = XmlHelper.ReadBool(node.Attributes["showSheetTabs"], blankValue: true);
		...
		cT_BookView.autoFilterDateGrouping = XmlHelper.ReadBool(node.Attributes["autoFilterDateGrouping"], blankValue: true);
	...
	{
		sw.Write($"<{nodeName}");
		if (visibility != ST_Visibility.visible)
		{
			XmlHelper.WriteAttribute(sw, "visibility", visibility.ToString());
		}
		XmlHelper.WriteAttribute(sw, "minimized", minimized, writeIfBlank: false);
