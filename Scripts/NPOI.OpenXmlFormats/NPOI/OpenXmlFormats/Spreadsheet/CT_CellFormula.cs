...
public class CT_CellFormula
{
	...
	{
		sw.Write($"<{nodeName}");
		if (t != ST_CellFormulaType.normal)
		{
			XmlHelper.WriteAttribute(sw, "t", t.ToString());
		}
		XmlHelper.WriteAttribute(sw, "aca", aca, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "ref", @ref);
		XmlHelper.WriteAttribute(sw, "dt2D", dt2D, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "dtr", dtr, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "del1", del1, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "del2", del2, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "r1", r1);
		XmlHelper.WriteAttribute(sw, "r2", r2);
		XmlHelper.WriteAttribute(sw, "ca", ca, writeIfBlank: false);
		if (t != ST_CellFormulaType.normal)
		{
			XmlHelper.WriteAttribute(sw, "si", si, writeIfBlank: true);
		}
		XmlHelper.WriteAttribute(sw, "bx", bx, writeIfBlank: false);
		if (!string.IsNullOrEmpty(valueField))
		{
			sw.Write(">");
			sw.Write(XmlHelper.EncodeXml(valueField).Replace("&quot;", "\""));
			sw.Write($"</{nodeName}>");
		}
		else
		{
	...
	public CT_CellFormula Copy()
	{
		return new CT_CellFormula
		{
			...
			refField = refField,
			siField = siField,
			siFieldSpecified = siFieldSpecified,
			tField = tField,
