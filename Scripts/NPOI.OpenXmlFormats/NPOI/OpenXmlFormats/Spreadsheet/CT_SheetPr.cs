...
public class CT_SheetPr
{
	...
	{
		sw.Write($"<{nodeName}");
		XmlHelper.WriteAttribute(sw, "syncHorizontal", syncHorizontal, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "syncVertical", syncVertical, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "syncRef", syncRef);
		XmlHelper.WriteAttribute(sw, "transitionEvaluation", transitionEvaluation, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "transitionEntry", transitionEntry, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "published", published, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "codeName", codeName);
		XmlHelper.WriteAttribute(sw, "filterMode", filterMode, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "enableFormatConditionsCalculation", enableFormatConditionsCalculation, writeIfBlank: false);
		sw.Write(">");
		if (tabColor != null)
		{
	...
	{
		CT_SheetPr cT_SheetPr = new CT_SheetPr();
		cT_SheetPr.codeNameField = codeNameField;
		cT_SheetPr.enableFormatConditionsCalculationField = enableFormatConditionsCalculationField;
		cT_SheetPr.filterModeField = filterModeField;
		cT_SheetPr.publishedField = publishedField;
		cT_SheetPr.syncHorizontalField = syncHorizontalField;
		cT_SheetPr.syncRefField = syncRefField;
		cT_SheetPr.syncVerticalField = syncVerticalField;
		cT_SheetPr.transitionEntryField = transitionEntryField;
		cT_SheetPr.transitionEvaluationField = transitionEvaluationField;
		if (outlinePrField != null)
		{
			cT_SheetPr.outlinePrField = outlinePrField.Clone();
		}
		if (pageSetUpPrField != null)
		{
			cT_SheetPr.pageSetUpPrField = pageSetUpPrField.Clone();
		}
		if (tabColorField != null)
		{
			cT_SheetPr.tabColorField = tabColorField.Copy();
		}
		return cT_SheetPr;
	}
