...
public class CT_String
{
	...
	public static CT_String Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_String.v = XmlHelper.ReadString(node.Attributes["v"]);
		if (node.Attributes["u"] != null)
		{
			cT_String.u = XmlHelper.ReadBool(node.Attributes["u"]);
		}
		if (node.Attributes["f"] != null)
		{
			cT_String.f = XmlHelper.ReadBool(node.Attributes["f"]);
		}
		cT_String.c = XmlHelper.ReadString(node.Attributes["c"]);
		if (node.Attributes["cp"] != null)
		{
			cT_String.cp = XmlHelper.ReadUInt(node.Attributes["cp"]);
		}
		if (node.Attributes["in"] != null)
		{
			cT_String.@in = XmlHelper.ReadUInt(node.Attributes["in"]);
		}
		cT_String.bc = XmlHelper.ReadBytes(node.Attributes["bc"]);
		cT_String.fc = XmlHelper.ReadBytes(node.Attributes["fc"]);
		if (node.Attributes["i"] != null)
		{
			cT_String.i = XmlHelper.ReadBool(node.Attributes["i"]);
		}
		if (node.Attributes["un"] != null)
		{
			cT_String.un = XmlHelper.ReadBool(node.Attributes["un"]);
		}
		if (node.Attributes["st"] != null)
		{
			cT_String.st = XmlHelper.ReadBool(node.Attributes["st"]);
		}
		if (node.Attributes["b"] != null)
		{
			cT_String.b = XmlHelper.ReadBool(node.Attributes["b"]);
		}
		cT_String.tpls = new List<CT_Tuples>();
		cT_String.x = new List<CT_X>();
		foreach (XmlNode childNode in node.ChildNodes)
		{
			if (childNode.LocalName == "tpls")
			{
				cT_String.tpls.Add(CT_Tuples.Parse(childNode, namespaceManager));
			}
			else if (childNode.LocalName == "x")
			{
				cT_String.x.Add(CT_X.Parse(childNode, namespaceManager));
			}
		}
		return cT_String;
	}
