...
public class MapInfoDocument
{
	...
	public void Save(Stream stream)
	{
		...
		if (map.Schema != null)
		{
			...
			{
				streamWriter.Write($"<Schema ID=\"{item.ID}\"");
				if (item.Namespace != null)
				{
					streamWriter.Write($" Namespace=\"{item.Namespace}\"");
				}
				if (item.SchemaRef != null)
				{
					streamWriter.Write($" SchemaRef=\"{item.SchemaRef}\"");
				}
				streamWriter.Write(">");
		...
		if (map.Map != null)
		{
			...
			{
				streamWriter.Write($"<Map ID=\"{item2.ID}\"");
				if (item2.SchemaID != null)
				{
					streamWriter.Write($" SchemaID=\"{item2.SchemaID}\"");
				}
				if (item2.RootElement != null)
				{
					streamWriter.Write($" RootElement=\"{item2.RootElement}\"");
				}
				if (item2.Name != null)
				{
					streamWriter.Write($" Name=\"{item2.Name}\"");
				}
