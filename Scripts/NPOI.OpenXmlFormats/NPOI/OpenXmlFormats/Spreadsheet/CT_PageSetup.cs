...
public class CT_PageSetup
{
	...
	public static CT_PageSetup Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_PageSetup.paperSize = XmlHelper.ReadUInt(node.Attributes["paperSize"], 1u);
		cT_PageSetup.scale = XmlHelper.ReadUInt(node.Attributes["scale"], 100u);
		cT_PageSetup.firstPageNumber = XmlHelper.ReadUInt(node.Attributes["firstPageNumber"], 1u);
		cT_PageSetup.fitToWidth = XmlHelper.ReadUInt(node.Attributes["fitToWidth"], 1u);
		cT_PageSetup.fitToHeight = XmlHelper.ReadUInt(node.Attributes["fitToHeight"], 1u);
		...
		cT_PageSetup.usePrinterDefaults = XmlHelper.ReadBool(node.Attributes["usePrinterDefaults"], blankValue: true);
		...
		cT_PageSetup.horizontalDpi = XmlHelper.ReadUInt(node.Attributes["horizontalDpi"], 600u);
		cT_PageSetup.verticalDpi = XmlHelper.ReadUInt(node.Attributes["verticalDpi"], 600u);
		cT_PageSetup.copies = XmlHelper.ReadUInt(node.Attributes["copies"], 1u);
	...
	{
		sw.Write($"<{nodeName}");
		XmlHelper.WriteAttribute(sw, "paperSize", paperSize, 1u);
		XmlHelper.WriteAttribute(sw, "scale", scale, 100u);
		XmlHelper.WriteAttribute(sw, "firstPageNumber", firstPageNumber, 1u);
		XmlHelper.WriteAttribute(sw, "fitToWidth", fitToWidth, 1u);
		XmlHelper.WriteAttribute(sw, "fitToHeight", fitToHeight, 1u);
		...
		XmlHelper.WriteAttribute(sw, "blackAndWhite", blackAndWhite, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "draft", draft, writeIfBlank: false);
		if (cellComments != ST_CellComments.none)
		{
			XmlHelper.WriteAttribute(sw, "cellComments", cellComments.ToString());
		}
		XmlHelper.WriteAttribute(sw, "useFirstPageNumber", useFirstPageNumber, writeIfBlank: false);
		if (errors != ST_PrintError.displayed)
		{
			XmlHelper.WriteAttribute(sw, "errors", errors.ToString());
		}
		XmlHelper.WriteAttribute(sw, "horizontalDpi", horizontalDpi, 600u);
		XmlHelper.WriteAttribute(sw, "verticalDpi", verticalDpi, 600u);
		XmlHelper.WriteAttribute(sw, "copies", copies, 1u);
