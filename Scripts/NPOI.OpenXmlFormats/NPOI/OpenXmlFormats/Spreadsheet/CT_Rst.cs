...
public class CT_Rst
{
				...
				if (rField != null && rField.Count > 0)
				{
					foreach (CT_RElt item in rField)
					{
						...
						if (item.rPr != null)
						{
							...
							if (item.rPr.u != null)
							{
								stringWriter.Write("<u val=\"" + item.rPr.u.val.ToString() + "\"/>");
							}
							if (item.rPr.color != null && item.rPr.color.theme != 0)
							{
								stringWriter.Write("<color theme=\"" + item.rPr.color.theme + "\"/>");
							}
							if (item.rPr.color != null && item.rPr.color.rgbSpecified)
							{
								stringWriter.Write("<color rgb=\"" + BitConverter.ToString(item.rPr.color.rgb).Replace("-", string.Empty) + "\"/>");
							}
							...
							{
								stringWriter.Write("<scheme val=\"" + item.rPr.scheme.val.ToString() + "\"/>");
							}
							if (item.rPr.sz != null)
							{
							...
							{
								stringWriter.Write("<vertAlign val=\"" + item.rPr.vertAlign.val.ToString() + "\"/>");
							}
							stringWriter.Write("</rPr>");
						}
						if (item.t != null)
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		if (t != null)
		{
			sw.Write($"<t xml:space=\"preserve\">{XmlHelper.ExcelEncodeString(XmlHelper.EncodeXml(t))}</t>");
		}
		if (r != null)
		{
