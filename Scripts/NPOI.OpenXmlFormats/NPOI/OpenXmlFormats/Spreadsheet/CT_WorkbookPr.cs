...
public class CT_WorkbookPr
{
	...
	public static CT_WorkbookPr Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_WorkbookPr.showBorderUnselectedTables = XmlHelper.ReadBool(node.Attributes["showBorderUnselectedTables"], blankValue: true);
		...
		cT_WorkbookPr.showInkAnnotation = XmlHelper.ReadBool(node.Attributes["showInkAnnotation"], blankValue: true);
		cT_WorkbookPr.backupFile = XmlHelper.ReadBool(node.Attributes["backupFile"]);
		cT_WorkbookPr.saveExternalLinkValues = XmlHelper.ReadBool(node.Attributes["saveExternalLinkValues"], blankValue: true);
		...
		cT_WorkbookPr.autoCompressPictures = XmlHelper.ReadBool(node.Attributes["autoCompressPictures"], blankValue: false);
	...
	{
		sw.Write($"<{nodeName}");
		XmlHelper.WriteAttribute(sw, "date1904", date1904, writeIfBlank: false);
		if (showObjects != ST_Objects.all)
		{
		...
		XmlHelper.WriteAttribute(sw, "filterPrivacy", filterPrivacy, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "promptedSolutions", promptedSolutions, writeIfBlank: false);
		if (!showInkAnnotationField)
		{
			XmlHelper.WriteAttribute(sw, "showInkAnnotation", showInkAnnotation, writeIfBlank: false);
		}
		XmlHelper.WriteAttribute(sw, "backupFile", backupFile, writeIfBlank: false);
		if (!saveExternalLinkValues)
		{
			XmlHelper.WriteAttribute(sw, "saveExternalLinkValues", saveExternalLinkValues);
		}
		if (updateLinks != ST_UpdateLinks.userSet)
		{
			XmlHelper.WriteAttribute(sw, "updateLinks", updateLinks.ToString());
		}
		XmlHelper.WriteAttribute(sw, "codeName", codeName);
		XmlHelper.WriteAttribute(sw, "hidePivotFieldList", hidePivotFieldList, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "showPivotChartFilter", showPivotChartFilter, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "allowRefreshQuery", allowRefreshQuery, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "publishItems", publishItems, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "checkCompatibility", checkCompatibility, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "autoCompressPictures", autoCompressPictures, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "refreshAllConnections", refreshAllConnections, writeIfBlank: false);
