...
public class CT_Row
{
	...
	public static CT_Row Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_Row.dyDescentField = XmlHelper.ReadDouble(node.Attributes["x14ac:dyDescent"]);
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		XmlHelper.WriteAttribute(sw, "customFormat", customFormat, writeIfBlank: false);
		if (ht >= 0.0)
		{
		...
		XmlHelper.WriteAttribute(sw, "thickBot", thickBot, writeIfBlank: false);
		XmlHelper.WriteAttribute(sw, "ph", ph, writeIfBlank: false);
