...
public class CT_Missing
{
	...
	public static CT_Missing Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		CT_Missing cT_Missing = new CT_Missing();
		if (node.Attributes["u"] != null)
		{
			cT_Missing.u = XmlHelper.ReadBool(node.Attributes["u"]);
		}
		if (node.Attributes["f"] != null)
		{
			cT_Missing.f = XmlHelper.ReadBool(node.Attributes["f"]);
		}
		cT_Missing.c = XmlHelper.ReadString(node.Attributes["c"]);
		if (node.Attributes["cp"] != null)
		{
			cT_Missing.cp = XmlHelper.ReadUInt(node.Attributes["cp"]);
		}
		if (node.Attributes["in"] != null)
		{
			cT_Missing.@in = XmlHelper.ReadUInt(node.Attributes["in"]);
		}
		cT_Missing.bc = XmlHelper.ReadBytes(node.Attributes["bc"]);
		cT_Missing.fc = XmlHelper.ReadBytes(node.Attributes["fc"]);
		if (node.Attributes["i"] != null)
		{
			cT_Missing.i = XmlHelper.ReadBool(node.Attributes["i"]);
		}
		if (node.Attributes["un"] != null)
		{
			cT_Missing.un = XmlHelper.ReadBool(node.Attributes["un"]);
		}
		if (node.Attributes["st"] != null)
		{
			cT_Missing.st = XmlHelper.ReadBool(node.Attributes["st"]);
		}
		if (node.Attributes["b"] != null)
		{
			cT_Missing.b = XmlHelper.ReadBool(node.Attributes["b"]);
		}
		cT_Missing.tpls = new List<CT_Tuples>();
		cT_Missing.x = new List<CT_X>();
		foreach (XmlNode childNode in node.ChildNodes)
		{
			if (childNode.LocalName == "tpls")
			{
				cT_Missing.tpls.Add(CT_Tuples.Parse(childNode, namespaceManager));
			}
			else if (childNode.LocalName == "x")
			{
				cT_Missing.x.Add(CT_X.Parse(childNode, namespaceManager));
			}
		}
		return cT_Missing;
	}
