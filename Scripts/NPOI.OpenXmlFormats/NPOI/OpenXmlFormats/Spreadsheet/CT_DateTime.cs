...
public class CT_DateTime
{
	...
	public static CT_DateTime Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		CT_DateTime cT_DateTime = new CT_DateTime();
		if (node.Attributes["v"] != null)
		{
			cT_DateTime.v = XmlHelper.ReadDateTime(node.Attributes["v"]);
		}
		if (node.Attributes["u"] != null)
		{
			cT_DateTime.u = XmlHelper.ReadBool(node.Attributes["u"]);
		}
		if (node.Attributes["f"] != null)
		{
			cT_DateTime.f = XmlHelper.ReadBool(node.Attributes["f"]);
		}
		cT_DateTime.c = XmlHelper.ReadString(node.Attributes["c"]);
		if (node.Attributes["cp"] != null)
		{
			cT_DateTime.cp = XmlHelper.ReadUInt(node.Attributes["cp"]);
		}
		cT_DateTime.x = new List<CT_X>();
		foreach (XmlNode childNode in node.ChildNodes)
		{
			if (childNode.LocalName == "x")
			{
				cT_DateTime.x.Add(CT_X.Parse(childNode, namespaceManager));
			}
		}
		return cT_DateTime;
	}
