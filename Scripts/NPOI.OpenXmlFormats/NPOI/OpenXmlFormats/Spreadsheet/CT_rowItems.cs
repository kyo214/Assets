...
public class CT_rowItems
{
	...
	public static CT_rowItems Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		CT_rowItems cT_rowItems = new CT_rowItems();
		if (node.Attributes["count"] != null)
		{
			cT_rowItems.count = XmlHelper.ReadUInt(node.Attributes["count"]);
		}
		cT_rowItems.i = new List<CT_I>();
		foreach (XmlNode childNode in node.ChildNodes)
		{
			if (childNode.LocalName == "i")
			{
				cT_rowItems.i.Add(CT_I.Parse(childNode, namespaceManager));
			}
		}
		return cT_rowItems;
	}
