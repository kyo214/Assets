...
public class CT_Anchor
{
	...
	public static CT_Anchor Parse(XmlNode node, XmlNamespaceManager namespaceManager)
	{
		...
		cT_Anchor.distT = XmlHelper.ReadUInt(node.Attributes["distT"]);
		cT_Anchor.distB = XmlHelper.ReadUInt(node.Attributes["distB"]);
		cT_Anchor.distL = XmlHelper.ReadUInt(node.Attributes["distL"]);
		cT_Anchor.distR = XmlHelper.ReadUInt(node.Attributes["distR"]);
		cT_Anchor.simplePos1 = XmlHelper.ReadBool(node.Attributes["simplePos"]);
		cT_Anchor.relativeHeight = XmlHelper.ReadUInt(node.Attributes["relativeHeight"]);
		cT_Anchor.behindDoc = XmlHelper.ReadBool(node.Attributes["behindDoc"]);
		cT_Anchor.locked = XmlHelper.ReadBool(node.Attributes["locked"]);
		cT_Anchor.layoutInCell = XmlHelper.ReadBool(node.Attributes["layoutInCell"]);
		cT_Anchor.hidden = XmlHelper.ReadBool(node.Attributes["hidden"]);
		cT_Anchor.allowOverlap = XmlHelper.ReadBool(node.Attributes["allowOverlap"]);
		if (node.Attributes["wp14:anchorId"] != null)
		{
			cT_Anchor.anchorIdField = XmlHelper.ReadBytes(node.Attributes["wp14:anchorId"]);
		}
		if (node.Attributes["wp14:editId"] != null)
		{
			cT_Anchor.editIdField = XmlHelper.ReadBytes(node.Attributes["wp14:editId"]);
		}
		foreach (XmlNode childNode in node.ChildNodes)
		{
			...
			{
				cT_Anchor.wrapSquare = CT_WrapSquare.Parse(childNode, namespaceManager);
			}
			else if (childNode.LocalName == "wrapNone")
			{
				cT_Anchor.wrapNone = new CT_WrapNone();
			}
			...
			{
				cT_Anchor.wrapTopAndBottom = CT_WrapTopBottom.Parse(childNode, namespaceManager);
			}
	...
	internal void Write(StreamWriter sw, string nodeName)
	{
		...
		XmlHelper.WriteAttribute(sw, "distT", distT, writeIfBlank: true);
		XmlHelper.WriteAttribute(sw, "distB", distB, writeIfBlank: true);
		XmlHelper.WriteAttribute(sw, "distL", distL);
		XmlHelper.WriteAttribute(sw, "distR", distR);
		XmlHelper.WriteAttribute(sw, "simplePos", simplePos1);
		XmlHelper.WriteAttribute(sw, "relativeHeight", relativeHeight);
		XmlHelper.WriteAttribute(sw, "behindDoc", behindDoc);
		XmlHelper.WriteAttribute(sw, "locked", locked);
		XmlHelper.WriteAttribute(sw, "layoutInCell", layoutInCell);
		if (!hidden)
		{
			XmlHelper.WriteAttribute(sw, "hidden", hidden);
		}
		XmlHelper.WriteAttribute(sw, "allowOverlap", allowOverlap);
		XmlHelper.WriteAttribute(sw, "wp14:anchorId", anchorIdField);
		XmlHelper.WriteAttribute(sw, "wp14:editId", editIdField);
		sw.Write(">");
		if (simplePos != null)
		{
		...
		{
			wrapTopAndBottom.Write(sw, "wrapTopAndBottom");
		}
		if (wrapSquare != null)
		{
			wrapSquare.Write(sw, "wrapSquare");
		}
		if (wrapNone != null)
		{
			wrapNone.Write(sw, "wrapNone");
		}
		if (docPr != null)
		{
