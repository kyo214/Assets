...
internal static class Helpers
{
	...
	public static CmsRecipientCollection DeepCopy(this CmsRecipientCollection recipients)
	{
		...
		{
			X509Certificate2 certificate = recipient2.Certificate;
			CmsRecipient recipient = new CmsRecipient(certificate: new X509Certificate2(certificate.Handle), recipientIdentifierType: recipient2.RecipientIdentifierType);
	...
	public static X509Certificate2 TryFindMatchingCertificate(this X509Certificate2Collection certs, SubjectIdentifier recipientIdentifier)
	{
		...
		case SubjectIdentifierType.IssuerAndSerialNumber:
		{
			...
			{
				if (AreByteArraysEqual(cert.GetSerialNumber(), ba2) && cert.Issuer == issuerName)
				{
					return cert;
				}
			}
		...
		case SubjectIdentifierType.SubjectKeyIdentifier:
		{
			...
			foreach (X509Certificate2 cert2 in certs)
			{
				byte[] subjectKeyIdentifier = PkcsPal.Instance.GetSubjectKeyIdentifier(cert2);
