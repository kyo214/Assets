...
internal class AsnReader
{
	...
	private int SeekEndOfContents(ReadOnlyMemory<byte> source)
	{
		...
		{
			Asn1Tag asn1Tag = asnReader.ReadTagAndLength(out var contentsLength, out var bytesRead);
	...
	{
		Asn1Tag tag = ReadTagAndLength(out var contentsLength, out var bytesRead);
	...
	{
		Asn1Tag tag = ReadTagAndLength(out var contentsLength, out headerLength);
	...
	{
		ReadOnlyMemory<byte> integerContents = GetIntegerContents(expectedTag, UniversalTagNumber.Integer, out var headerLength);
	...
	{
		ReadOnlyMemory<byte> integerContents = GetIntegerContents(expectedTag, tagNumber, out var headerLength);
	...
	{
		bytesWritten = ProcessConstructedBitString(source, destination, (ReadOnlyMemory<byte> value, byte lastByte, Span<byte> dest) =>
		{
	...
	{
		bool flag = TryGetPrimitiveBitStringValue(expectedTag, out var _, out var _, out var headerLength, out unusedBitCount, out value, out var normalizedLastByte);
		if (flag)
		{
	...
	public bool TryCopyBitStringBytes(Asn1Tag expectedTag, Span<byte> destination, out int unusedBitCount, out int bytesWritten)
	{
		...
		bool num = TryCopyConstructedBitStringValue(Slice(_data, headerLength, contentsLength), destination, !contentsLength.HasValue, out unusedBitCount, out var bytesRead, out bytesWritten);
		if (num)
		{
	...
	{
		ReadOnlyMemory<byte> integerContents = GetIntegerContents(expectedTag, UniversalTagNumber.Enumerated, out var headerLength);
		_data = _data.Slice(headerLength + integerContents.Length);
		return integerContents;
	}
	...
	{
		int num = CopyConstructedOctetString(source, Span<byte>.Empty, write: false, isIndefinite, out var _);
		if (_ruleSet == AsnEncodingRules.CER && num <= 1000)
		{
			throw new CryptographicException("ASN1 corrupted data.");
		}
	...
	public bool TryCopyOctetStringBytes(Asn1Tag expectedTag, Span<byte> destination, out int bytesWritten)
	{
		...
		bool num = TryCopyConstructedOctetStringContents(Slice(_data, headerLength, contentLength), destination, !contentLength.HasValue, out var bytesRead, out bytesWritten);
		if (num)
		{
			_data = _data.Slice(headerLength + bytesRead);
		}
	...
	{
		string result = ReadObjectIdentifierAsString(expectedTag, out var totalBytesRead);
	...
	{
		string text = ReadObjectIdentifierAsString(expectedTag, out var totalBytesRead);
	...
	private bool TryCopyCharacterStringBytes(Asn1Tag expectedTag, UniversalTagNumber universalTagNumber, Span<byte> destination, out int bytesRead, out int bytesWritten)
	{
		...
		bool num = TryCopyConstructedOctetStringContents(Slice(_data, headerLength, contentLength), destination, !contentLength.HasValue, out var bytesRead2, out bytesWritten);
		if (num)
		{
	...
	{
		byte[] rented = null;
		ReadOnlySpan<byte> octetStringContents = GetOctetStringContents(expectedTag, universalTagNumber, out var bytesRead, ref rented);
		try
		{
			string result;
	...
	{
		CheckCharacterStringEncodingType(encodingType);
		bool num = TryCopyCharacterStringBytes(expectedTag, encodingType, destination, out var bytesRead, out bytesWritten);
		if (num)
		{
			_data = _data.Slice(bytesRead);
		}
	...
	public DateTimeOffset GetUtcTime(Asn1Tag expectedTag, int twoDigitYearMax = 2049)
	{
		...
		ReadOnlySpan<byte> octetStringContents = GetOctetStringContents(expectedTag, UniversalTagNumber.UtcTime, out var bytesRead, ref rented, tmpSpace);
	...
	{
		byte[] rented = null;
		ReadOnlySpan<byte> octetStringContents = GetOctetStringContents(expectedTag, UniversalTagNumber.GeneralizedTime, out var bytesRead, ref rented);
