...
internal static class AsnSerializer
{
	...
	{
		return s_orderedFields.GetOrAdd(typeT, (Type t) =>
		{
	...
	{
		Serializer literalValueSerializer = GetSimpleSerializer(typeT, fieldInfo, out var defaultContents, out var isOptional, out var explicitTag);
		Serializer serializer = literalValueSerializer;
		if (isOptional)
		{
			serializer = (object obj, AsnWriter writer) =>
			{
				if (obj != null)
				{
		...
		{
			return (object obj, AsnWriter writer) =>
			{
	...
	private static Serializer GetSimpleSerializer(Type typeT, FieldInfo fieldInfo, out byte[] defaultContents, out bool isOptional, out Asn1Tag? explicitTag)
	{
		...
		if (typeT.IsEnum)
		{
			...
			{
				return (object value, AsnWriter writer) =>
				{
	...
	{
		Deserializer deserializer = GetSimpleDeserializer(typeT, fieldInfo, out var fieldData);
		if (fieldData.HasExplicitTag)
		{
