...
public sealed class SignedCms
{
	...
	{
		ContentInfoAsn contentInfoAsn = AsnSerializer.Deserialize<ContentInfoAsn>(encodedMessage, AsnEncodingRules.BER, out var _);
	...
	public void ComputeSignature(CmsSigner signer, bool silent)
	{
		...
		SignerInfoAsn signerInfoAsn = signer.Sign(data, text, silent, out var chainCerts);
		bool flag = false;
		if (!_hasData)
		{
			flag = true;
	...
	private static void CheckSignatures(SignerInfoCollection signers, X509Certificate2Collection extraStore, bool verifySignatureOnly)
	{
		...
		foreach (SignerInfo signer in signers)
		{
			...
			SignerInfoCollection counterSignerInfos = signer.CounterSignerInfos;
	...
	public void CheckHash()
	{
		...
		foreach (SignerInfo item in signerInfos)
		{
			if (item.SignerIdentifier.Type == SubjectIdentifierType.NoSignature)
			{
				item.CheckHash();
			}
		}
