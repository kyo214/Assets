...
public sealed class SignerInfo
{
	...
	public void ComputeCounterSignature(CmsSigner signer)
	{
		...
		SignerInfoAsn value = signer.Sign(signerInfo._signature, null, silent: false, out var chainCerts);
	...
	private static X509Certificate2 FindSignerCertificate(SubjectIdentifier signerIdentifier, X509Certificate2Collection extraStore)
	{
		...
		case SubjectIdentifierType.IssuerAndSerialNumber:
		{
			...
			{
				if (item.IssuerName.Name == x509IssuerSerial.IssuerName)
				{
		...
		{
			foreach (X509Certificate2 item2 in x509Certificate2Collection)
			{
				if (item2 != x509Certificate)
				{
					item2.Dispose();
				}
			}
	...
	private void Verify(X509Certificate2Collection extraStore, X509Certificate2 certificate, bool verifySignatureOnly)
	{
		...
		foreach (X509Extension extension in certificate.Extensions)
		{
			...
			{
				X509KeyUsageExtension x509KeyUsageExtension = extension as X509KeyUsageExtension;
				if (x509KeyUsageExtension == null)
				{
					x509KeyUsageExtension = new X509KeyUsageExtension();
					x509KeyUsageExtension.CopyFrom(extension);
				}
