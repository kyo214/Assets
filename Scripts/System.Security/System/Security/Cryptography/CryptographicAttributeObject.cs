...
public sealed class CryptographicAttributeObject
{
	...
	public CryptographicAttributeObject(Oid oid, AsnEncodedDataCollection values)
	{
		...
		foreach (AsnEncodedData value in values)
		{
			...
			{
				throw new InvalidOperationException(global::SR.Format("AsnEncodedData element in the collection has wrong Oid value: expected = '{0}', actual = '{1}'.", oid.Value, value.Oid.Value));
			}
		}
		Values = values;
	}
