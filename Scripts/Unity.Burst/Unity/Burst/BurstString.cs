...
internal static class BurstString
{
	...
	private unsafe static uint Dragon4(ulong mantissa, int exponent, uint mantissaHighBitIdx, bool hasUnequalMargins, CutoffMode cutoffMode, uint cutoffNumber, byte* pOutBuffer, uint bufferSize, out int pOutExponent)
	{
		...
		tBigInt pResult = default;
		tBigInt pResult2 = default;
		tBigInt input = default;
		tBigInt pResult3 = default;
		tBigInt* ptr2;
	...
	private unsafe static int FormatPositional(byte* pOutBuffer, uint bufferSize, ulong mantissa, int exponent, uint mantissaHighBitIdx, bool hasUnequalMargins, int precision)
	{
		...
		uint num2 = ((precision >= 0) ? Dragon4(mantissa, exponent, mantissaHighBitIdx, hasUnequalMargins, CutoffMode.FractionLength, (uint)precision, pOutBuffer, num, out var pOutExponent) : Dragon4(mantissa, exponent, mantissaHighBitIdx, hasUnequalMargins, CutoffMode.Unique, 0u, pOutBuffer, num, out pOutExponent));
		uint num3 = 0u;
		if (pOutExponent >= 0)
		{
	...
	{
		uint num = ((precision >= 0) ? Dragon4(mantissa, exponent, mantissaHighBitIdx, hasUnequalMargins, CutoffMode.TotalLength, (uint)(precision + 1), pOutBuffer, bufferSize, out var pOutExponent) : Dragon4(mantissa, exponent, mantissaHighBitIdx, hasUnequalMargins, CutoffMode.Unique, 0u, pOutBuffer, bufferSize, out pOutExponent));
		byte* ptr = pOutBuffer;
		if (bufferSize > 1)
		{
			ptr++;
	...
	private unsafe static void ConvertFloatToString(byte* dest, ref int destIndex, int destLength, float value, FormatOptions formatOptions)
	{
		...
		uint num4 = Dragon4(num, exponent2, mantissaHighBitIdx, hasUnequalMargins, CutoffMode.TotalLength, (uint)num2, ptr, (uint)(num3 - 1), out var pOutExponent);
		ptr[num4] = 0;
