...
public static class X86
{
	...
	public enum MXCSRBits
	{
		...
		RoundDown = 0x2000,
		RoundUp = 0x4000,
		RoundTowardZero = RoundingControlMask,
	...
	public enum RoundingMode
	{
		...
		FROUND_TO_NEG_INF = 1,
		FROUND_TO_POS_INF = 2,
		FROUND_TO_ZERO = FROUND_TO_NEG_INF | FROUND_TO_POS_INF,
		FROUND_CUR_DIRECTION = 4,
		FROUND_RAISE_EXC = 0,
		FROUND_NO_EXC = 8,
		FROUND_NINT = 0,
		FROUND_FLOOR = FROUND_TO_NEG_INF,
		FROUND_CEIL = FROUND_TO_POS_INF,
		FROUND_TRUNC = FROUND_TO_ZERO,
		FROUND_RINT = FROUND_CUR_DIRECTION,
		FROUND_NEARBYINT = FROUND_CUR_DIRECTION | FROUND_NO_EXC,
		FROUND_NINT_NOEXC = FROUND_NO_EXC,
		...
		FROUND_RINT_NOEXC = FROUND_NEARBYINT
	}
	...
	public static class Sse4_2
	{
		...
		public enum SIDD
		{
			...
			UWORD_OPS = 1,
			SBYTE_OPS = 2,
			SWORD_OPS = UWORD_OPS | SBYTE_OPS,
			CMP_EQUAL_ANY = 0,
			CMP_RANGES = 4,
			CMP_EQUAL_EACH = 8,
			CMP_EQUAL_ORDERED = CMP_RANGES | CMP_EQUAL_EACH,
			POSITIVE_POLARITY = 0,
			NEGATIVE_POLARITY = 0x10,
			MASKED_POSITIVE_POLARITY = 0x20,
			MASKED_NEGATIVE_POLARITY = NEGATIVE_POLARITY | MASKED_POSITIVE_POLARITY,
			LEAST_SIGNIFICANT = 0,
			MOST_SIGNIFICANT = 0x40,
			BIT_MASK = 0,
			UNIT_MASK = MOST_SIGNIFICANT
		}
		...
		private unsafe static int ComputeStringLength<T>(T* ptr, int max) where T : unmanaged, IEquatable<T>
		{
			for (int i = 0; i < max; i++)
			{
				if (EqualityComparer<T>.Default.Equals(ptr[i], default))
				{
					return i;
				}
			}
		...
		private unsafe static int ComputeStrCmpIntRes2<T>(T* a, int alen, T* b, int blen, int len, int imm8, int allOnes) where T : unmanaged, IComparable<T>, IEquatable<T>
		{
			bool flag = false;
			bool flag2 = false;
			StrBoolArray strBoolArray = default;
			for (int i = 0; i < len; i++)
			{
				T val = a[i];
				if (i == alen)
			...
			for (int i = 0; i < len; i++)
			{
				if ((imm8 & 0x10) != 0)
				{
					if ((imm8 & 0x20) != 0)
					{
						if (EqualityComparer<T>.Default.Equals(b[i], default))
						{
							flag2 = true;
						}
